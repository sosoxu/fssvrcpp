...
struct UseCasePorts {
  ...
  //  ★ C9.26：**测试专用**的确定性崩溃窗口接缝（环境变量 `FSS_CLAIM_HOLD_MS`，
  //    **不是配置键**，见 docs/runbook.md §10.2）。
  //    > 0 时 `CreateFileMetadata` 在"原子领取成功 + 在途租约就绪"之后、**复制之前**
  //    阻塞该毫秒数，然后照常继续；`0`（默认）= 与接线前**逐字一致**（不 sleep）。
  //    为什么必须有它：C9.26 的"两进程 + 真实 kill -9"用例要断言"进程**确实**持着
  //    `claiming` 行时被杀"，而没有接缝时这个窗口只有毫秒级 —— 用固定 sleep 去赌时序
