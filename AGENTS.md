...
| **阶段 6（元数据记录语义完整化）** | 🚧 **进行中（切片 6/7）**：切片 1 = `src/infra/metadata/sqlite/sqlite_metadata_repository.{h,cpp}`（版本链 + `is_latest` **部分唯一索引** + partition 隔离 + `data` 列无损往返），元数据契约在 SQLite 上**跑第二遍**（闭合 C2.10 元数据侧）、C6.5 满足；切片 2 = **校验和 C6.4**（增量 `crypto::Hasher`（RFC/FIPS 公开向量兜底）+ 算法解析/规范名/hex 结构校验 + 第 7 步"原生可用则采用、不可用则**流式回算** SHA-256" + **覆写**客户端传入的两处 —— 上游语义，**不是**"比对不符就 400"），并证 64 MiB 跨 store 搬迁 + 回算的 RSS 增长 80 KiB（含"整块读回"自证对照 65664 KiB）；切片 3 = **12 步序列 C6.3**（统一的第 12 步回滚 `RollbackCreatedObject`、**第 7 步失败漏回滚已修**、补发第 10 步的第二个事件 `datasetDetails`（`correlationId` 由 `x-correlation-id` 头透传）、第 11 步清理失败补审计告警；正常路径顺序 + **7 个故障注入点** + 两处自证对照），切片 4 = **`getFileList` C6.6 + 角色 C6.8**（上游三条验收 fixture 逐字驱动；`Items` 缺省 0、`Driver` 小写、无记录消息对齐上游；9 个角色常量集中为唯一真相 + `AuthorizeAny` "任一角色" + 端点↔角色映射测试 + "403 先于 400"；并修 `ParseIso8601` 的**越界时间静默归一化**），切片 5 = **DMS 6 端点 + Delivery C6.7**（上传/下载位置的两套键集合：files 用 `fileSource`，file-collections 用 `fileCollectionSource` + `fileCount`/`fileNames`；`retrievalProperties` 补齐 `fileSource`/`createdBy`/`expiryTime`；上游 DMS 端到端"上传→登记→取回字节一致"），切片 6 = **大文件 C6.9 + 幂等并发 C6.11 + tmp 名 C6.13**（1 GiB 搬迁 RSS 增长 3432 KiB < 64 MiB；内存装配与 **SQLite 两个连接**并发提交同一 FileSource 都"恰 1 条记录、1 份对象"，含"朴素主键 check-then-insert 必现重复"的对照；POSIX 临时名序号改为**进程级**，含"朴素临时名必错乱"的对照；顺带修掉三个内存适配器的**线程安全**与两处"持锁后互调"的**自死锁**），`ctest -L phase6` **8 测试 / 2447 断言**。剩余：**GC 租约 C6.12** + C6.10 回归、远端仓储、组合根改接 SQLite 元数据仓储 |
| 已有产品代码 | `src/common/`（L1 十二个模块）+ `src/domain/model/`（`dataset--File.Generic` 全字段模型）+ `src/domain/ports/`（15 个端口）+ `src/app/services/`（`expiry_policy`/`kind_validator`/`object_key_policy`/`location_issuer`）+ `src/app/usecases/`（13 个用例）+ `src/infra/blob/posix/`（POSIX 驱动）+ `src/infra/blob/s3/`（SigV4 + S3 驱动）+ `src/infra/location/sqlite/`（SQLite 位置仓储）+ `src/infra/metadata/sqlite/`（SQLite 元数据仓储）+ `src/infra/io/`（I/O 引擎 + 落盘接缝）+ `src/infra/transfer/`（自签 token + 数据面内核）+ `src/infra/{blob,location,metadata}/memory/`（内存适配器）+ `src/adapters/http/`（REST 适配层：错误映射 + DTO + 路由）+ `src/main/server_main.cpp`（组合根）+ `tests/framework/{port_contract.h,fake_ports.h}`（契约测试基类 + 端口替身） |
| 当前可运行的验证 | `./scripts/check_docs.sh`（D1~D5）· `ctest -L phase0`（55 断言）· `scripts/verify_guard.sh`（护栏自证）· `scripts/verify_http_hardening.sh`（H-2 防护自证）· `scripts/verify_link_graph.sh`（C2.1 链接图，含越层注入自证）· `scripts/verify_capability_guard.sh`（C2.7 能力护栏自证）· `scripts/verify_composition_root.sh`（C4.9 组合根护栏自证）· `scripts/verify_driver_switch.sh`（C5.9：同一二进制 posix/s3 各跑一遍）· `scripts/verify_transfer_no_timeout.sh`（C4.11 的 ≥5 分钟慢传输实测，非日常门槛）· `ctest -L phase1`（14 测试 / 4984 断言）· `ctest -L phase2`（10 测试 / 952 断言）· `ctest -L phase3`（9 测试 / 580 断言）· `ctest -L phase4`（8 测试 / 1058 断言）· `ctest -L phase5`（6 测试 / 1737 断言）· `ctest -L phase6`（8 测试 / 2447 断言）· `scripts/run_sanitizers.sh`（ASan+UBSan+LSan，phase0~6 全绿）· `scripts/bench_logging.sh`（日志热路径基准）· `FSS_GATES_WITH_PG=1`（+PG 基建 5 项） |
...
P6 元数据语义完整化      🚧 进行中（切片 6/7：… + 大文件/幂等并发/tmp 名）
P7 gRPC 适配层 + 双协议等价性
P8 认证授权与多租户
P9 硬化与交付
```
