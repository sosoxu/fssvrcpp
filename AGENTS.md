...
P7 gRPC 适配层 + 双协议等价性   🚧 进行中（切片 2/3：14 个一元 RPC + 契约 §6 矩阵 12 行 + json_name 对齐）
P8 认证授权与多租户
P9 硬化与交付
```
