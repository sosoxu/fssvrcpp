...
| **阶段 9（硬化与交付）** | 🚧 **进行中（切片 1/3 完成 + 切片 2 全部完成：指标 / GC / 性能基线 / ADR-006）**：① **C9.1 并发**（100 并发 JSON 零 5xx；8 并发大文件逐路 SHA-256 + 存储侧 checksum 三方一致）；② **C9.2 故障注入 + 恢复 ≤30 秒**（存储不可用 503 / 元数据失败 500 / 磁盘满含回滚 502 / 远端鉴权依赖 503 + 两类非致命依赖；远端依赖用 `mock_entitlements --fail-file` 注入与恢复）；③ **C9.3 六类资源上限**各一条"被拒绝" + 对照；④ **C9.6 指标**（`/metrics` 暴露 HTTP+存储+GC 全部指标、格式合法、值真的动过、**不含 secret**）；⑤ **C9.5/C9.25 GC 清理**（`IBlobStore::remove_temp_files`：够旧的 `.tmp_*` 删、**在途的保护且计数可观测**、被位置记录"引用"也照删）；⑥ 修掉 **P9-D01~D09**（测试替身非线程安全；POSIX `last_write_time` 时基错 → GC 误删在途对象；数据面错误码被"读体失败 400"覆盖；流式 handler 不读体却回 2xx；S3 `list/stat` 时间戳恒 0；护栏白名单）。`ctest -L phase9` **5 测试 / 451 断言**。⑦ **C9.4/C9.11/C9.13/C9.15 容量基线**（独立进程 + 互不重叠绑核；每点位 **3 次取中位数**；`bench/capacity_bench.cpp` + `scripts/bench_baseline.sh` + `docs/appendix/capacity-baseline/BASELINE.tsv`）：控制面读 6809/13849/12636/11820 req/s（c1/c4/c16/c64）、数据面小对象 16678 @c16、大文件单流 1343 MiB/s、上传链 43.3→189.1 files/s、**fsync 档位 109.9 / 468.6 / 452.0 files/s（4.3x）**、SQLite 写 8 线程 2430 writes/s（1 线程为报告型：本机不可复现）；含 **p99 阈值**与**回归 >20% 判失败**（⚠️ 本机跨会话漂移实测 ≈40%（control_read_c4 = 14308/14140/9969/13849），该判据只在同一会话/独占硬件上有判定力；且**批量删除必须与测量解耦**（实测把写入点位 p99 从 7~44 ms 抬到 293~890 ms）—— 均已如实登记）；⑧ **C9.12/C9.16 ADR-006 定稿**：同一负载生成器受控复核 httplib 内容提供者 vs 裸 `sendfile` = **2.12x / 2.02x（两次）≥ 1.5x → 采纳 sendfile 方向**（实现未交付，边界条件见 ADR-006 §4）；组合根顺带接上 `storage.posix.durability` 三档与 **SQLite 元数据仓储**。`ctest -L phase9` **5 测试 / 451 断言**。剩余：切片 3 = P9 纳入门槛（C9.7）+ 镜像（C9.8）+ 运维文档（C9.9）+ 风险收口（C9.10） |
...
P9 硬化与交付   🚧 进行中（切片 1/3 完成 + 切片 2 全部完成：并发 C9.1 + 故障注入 C9.2 + 资源上限 C9.3 + 指标 C9.6 + GC 清理 C9.5/C9.25 + 容量基线 C9.4/C9.11/C9.13/C9.15 + ADR-006 定稿 C9.12/C9.16）
```
