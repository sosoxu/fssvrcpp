...
| **阶段 6（元数据记录语义完整化）** | 🚧 **进行中（切片 3/6）**：切片 1 = `src/infra/metadata/sqlite/sqlite_metadata_repository.{h,cpp}`（版本链 + `is_latest` **部分唯一索引** + partition 隔离 + `data` 列无损往返），元数据契约在 SQLite 上**跑第二遍**（闭合 C2.10 元数据侧）、C6.5 满足；切片 2 = **校验和 C6.4**（增量 `crypto::Hasher`（RFC/FIPS 公开向量兜底）+ 算法解析/规范名/hex 结构校验 + 第 7 步"原生可用则采用、不可用则**流式回算** SHA-256" + **覆写**客户端传入的两处 —— 上游语义，**不是**"比对不符就 400"），并证 64 MiB 跨 store 搬迁 + 回算的 RSS 增长 80 KiB（含"整块读回"自证对照 65664 KiB）；切片 3 = **12 步序列 C6.3**（统一的第 12 步回滚 `RollbackCreatedObject`、**第 7 步失败漏回滚已修**、补发第 10 步的第二个事件 `datasetDetails`（`correlationId` 由 `x-correlation-id` 头透传）、第 11 步清理失败补审计告警；正常路径顺序 + **7 个故障注入点** + 两处自证对照），`ctest -L phase6` **3 测试 / 610 断言**。剩余：`getFileList`（C6.6）、角色常量（C6.8）、GC 租约/幂等并发/tmp 名（C6.11~C6.13）、DMS/Delivery（C6.7）、**1 GiB** 搬迁 RSS 绝对上限（C6.9）、远端仓储、组合根改接 SQLite 元数据仓储 |
...
| 当前可运行的验证 | `./scripts/check_docs.sh`（D1~D5）· `ctest -L phase0`（55 断言）· `scripts/verify_guard.sh`（护栏自证）· `scripts/verify_http_hardening.sh`（H-2 防护自证）· `scripts/verify_link_graph.sh`（C2.1 链接图，含越层注入自证）· `scripts/verify_capability_guard.sh`（C2.7 能力护栏自证）· `scripts/verify_composition_root.sh`（C4.9 组合根护栏自证）· `scripts/verify_driver_switch.sh`（C5.9：同一二进制 posix/s3 各跑一遍）· `scripts/verify_transfer_no_timeout.sh`（C4.11 的 ≥5 分钟慢传输实测，非日常门槛）· `ctest -L phase1`（14 测试 / 4970 断言）· `ctest -L phase2`（10 测试 / 952 断言）· `ctest -L phase3`（9 测试 / 580 断言）· `ctest -L phase4`（8 测试 / 1058 断言）· `ctest -L phase5`（6 测试 / 1737 断言）· `ctest -L phase6`（3 测试 / 610 断言）· `scripts/run_sanitizers.sh`（ASan+UBSan+LSan，phase0/1/2/3/4 全绿）· `scripts/bench_logging.sh`（日志热路径基准）· `FSS_GATES_WITH_PG=1`（+PG 基建 5 项） |
...
P6 元数据语义完整化      🚧 进行中（切片 3/6：元数据仓储 + 校验和 C6.4 + 12 步序列 C6.3）
P7 gRPC 适配层 + 双协议等价性
P8 认证授权与多租户
P9 硬化与交付
```
