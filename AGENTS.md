...
| 阶段 4 后续（**已在阶段 10 接入**） | 组合根接 `config/fss.example.json`：**阶段 10 切片 1/2 已实现**（CLI > env > file > 默认；`--config`/`--set`/`--print-config`/`--once`）。156 个叶子键按**三态**逐键登记：**生效 72 / 拒绝启动 16 / 已读但无效果 68**（`docs/operations.md` §1.2/§1.3）；生产环境有"必须 jwt + 必须验签 + 密钥非空"的启动强校验 |
...
P10 配置面接线   🚧 进行中（切片 1/2：--config/--set/--print-config/--once + 优先级与来源可见 + server.http/storage/auth/observability 接通 + 生产强校验 + GC 周期调度与 expiry 接通 + 16 个未实现键显式拒绝 + 156 键三态 72/16/68 + 新测试 16 用例/288 断言）
```
