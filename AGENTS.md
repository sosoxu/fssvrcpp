...
| **阶段 8（认证授权与多租户）** | 🚧 **进行中（切片 2/3）**：切片 1 = 本地 JWT 授权器（`src/infra/auth/local/`，ADR-012：HS256 + `exp`/`nbf`/`iss`/`aud` + 角色 claim ∪ 静态角色表 + **租户绑定**，fail-closed）+ HTTP 适配层的**路由级鉴权预检**（`RouteAuthTable()`，鉴权先于 DTO 解析，未登记路由 fail-closed）+ 配置强校验 + `/v2/info` 的 `authMode`；切片 2 = **跨租户隔离**（两租户 + 真实 HTTP/JWT：跨租户头 → 403、B 读/删/列 A 的记录 → 404/空集、签名 URL 绑对象）+ **远端 Entitlements**（`src/infra/auth/remote/`，超时/5xx/坏 JSON/连不上/未配地址 → **一律 503**，`fail_closed=false` 被配置校验拒绝）+ 组合根 `FSS_AUTH_MODE=remote-entitlements`；`ctest -L phase8` **4 测试 / 954 断言**。剩余：切片 3 = 事件/审计（C8.6/C8.7）+ `deployment.mode=multi` 启动校验（C8.9）+ 时钟偏移（C8.10） |
...
P8 认证授权与多租户   🚧 进行中（切片 2/3：+ 跨租户隔离 + 远端 Entitlements fail-closed）
P9 硬化与交付
```
