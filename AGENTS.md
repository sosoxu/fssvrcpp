...
| 事实 | 现状 |
| --- | --- |
| **配置键三态** | **157** 个叶子键：**生效 129 / 拒绝启动 15 / 已读但无效果 13**（B2b 后；B2a 新增 `deployment.expected_instances` 并让 `metadata.postgres.schema_version_check`、`deployment.clock_skew_tolerance_seconds` 转生效，B2b 让 `storage.posix.shared_mount_required` 转生效）。逐键见 [docs/operations.md](docs/operations.md) §1.3，由 `test_operations_doc` 自动比对 |
