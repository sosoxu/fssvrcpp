...
| **阶段 6（元数据记录语义完整化）** | 🚧 **进行中（切片 1/6）**：`src/infra/metadata/sqlite/sqlite_metadata_repository.{h,cpp}`（版本链 + `is_latest` **部分唯一索引** + partition 隔离 + `data` 列无损往返）；元数据契约在 SQLite 上**跑第二遍**（闭合 C2.10 元数据侧）、C6.5 版本链满足；`ctest -L phase6` 1 测试 / 122 断言。剩余：校验和（C6.4）、12 步序列故障注入（C6.3）、`getFileList`（C6.6）、角色常量（C6.8）、GC 租约/幂等并发/tmp 名（C6.11~C6.13）、DMS/Delivery（C6.7）、大文件搬迁 RSS（C6.9）、远端仓储 |
...
P6 元数据语义完整化      🚧 进行中（切片 1/6：SQLite 元数据仓储 + 元数据契约第二遍）
P7 gRPC 适配层 + 双协议等价性
P8 认证授权与多租户
P9 硬化与交付
```
