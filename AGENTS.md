...
| **阶段 9（硬化与交付）** | 🚧 **进行中（切片 1/3 完成 + 切片 2 的指标/GC 部分）**：① **C9.1 并发**（100 并发 JSON 零 5xx；8 并发大文件逐路 SHA-256 + 存储侧 checksum 三方一致）；② **C9.2 故障注入 + 恢复 ≤30 秒**（存储不可用 503 / 元数据失败 500 / 磁盘满含回滚 502 / 远端鉴权依赖 503 + 两类非致命依赖；远端依赖用 `mock_entitlements --fail-file` 注入与恢复）；③ **C9.3 六类资源上限**各一条"被拒绝" + 对照；④ **C9.6 指标**（`/metrics` 暴露 HTTP+存储+GC 全部指标、格式合法、值真的动过、**不含 secret**）；⑤ **C9.5/C9.25 GC 清理**（`IBlobStore::remove_temp_files`：够旧的 `.tmp_*` 删、**在途的保护且计数可观测**、被位置记录"引用"也照删）；⑥ 修掉 **P9-D01~D09**（测试替身非线程安全；POSIX `last_write_time` 时基错 → GC 误删在途对象；数据面错误码被"读体失败 400"覆盖；流式 handler 不读体却回 2xx；S3 `list/stat` 时间戳恒 0；护栏白名单）。`ctest -L phase9` **5 测试 / 451 断言**。剩余：切片 2 = 性能基线（C9.4/C9.11/C9.13/C9.15）+ ADR-006 复核（C9.12/C9.16）；切片 3 = P9 纳入门槛（C9.7）+ 镜像（C9.8）+ 运维文档（C9.9）+ 风险收口（C9.10） |
...
P9 硬化与交付   🚧 进行中（切片 1/3 + 切片 2 的指标/GC：并发 C9.1 + 故障注入 C9.2 + 资源上限 C9.3 + 指标 C9.6 + GC 清理 C9.5/C9.25）
```
