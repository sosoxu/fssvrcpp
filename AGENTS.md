...
| 阶段 4 后续（**已在阶段 10 接入**） | 组合根接 `config/fss.example.json`：**阶段 10 切片 1/2/3 已实现**（CLI > env > file > 默认；`--config`/`--set`/`--print-config`/`--once`）。156 个叶子键按**三态**逐键登记：**生效 82 / 拒绝启动 18 / 已读但无效果 56**（`docs/operations.md` §1.2/§1.3；切片 3 接通 9 键、C10.16 接通 1 键 + 2 键改为拒绝启动）；生产环境有"必须 jwt + 必须验签 + 密钥非空"的启动强校验 |
...
P10 配置面接线   ✅ 切片 1/2/3 全部完成（--config/--set/--print-config/--once + 优先级与来源可见 + server.http/storage/auth/observability 接通 + 生产强校验 + GC 周期调度与 expiry 接通 + 16 个未实现键显式拒绝 + **C10.16**：`partition.file.opendes.max_file_bytes` 超限 → 413、校验算法非法 → exit 78 + 156 键三态 **生效 82 / 拒绝启动 18 / 已读但无效果 56** + 新测试 22 用例；`storage.posix.*` 细节键与容器名无字段可接，如实登记）
```
