...
| **阶段 6（元数据记录语义完整化）** | 🚧 **进行中（切片 4/6）**：切片 1 = `src/infra/metadata/sqlite/sqlite_metadata_repository.{h,cpp}`（版本链 + `is_latest` **部分唯一索引** + partition 隔离 + `data` 列无损往返），元数据契约在 SQLite 上**跑第二遍**（闭合 C2.10 元数据侧）、C6.5 满足；切片 2 = **校验和 C6.4**（增量 `crypto::Hasher`（RFC/FIPS 公开向量兜底）+ 算法解析/规范名/hex 结构校验 + 第 7 步"原生可用则采用、不可用则**流式回算** SHA-256" + **覆写**客户端传入的两处 —— 上游语义，**不是**"比对不符就 400"），并证 64 MiB 跨 store 搬迁 + 回算的 RSS 增长 80 KiB（含"整块读回"自证对照 65664 KiB）；切片 3 = **12 步序列 C6.3**（统一的第 12 步回滚 `RollbackCreatedObject`、**第 7 步失败漏回滚已修**、补发第 10 步的第二个事件 `datasetDetails`（`correlationId` 由 `x-correlation-id` 头透传）、第 11 步清理失败补审计告警；正常路径顺序 + **7 个故障注入点** + 两处自证对照），`ctest -L phase6` **5 测试 / 997 断言**。切片 4 = **`getFileList` C6.6 + 角色 C6.8**（上游三条验收 fixture 逐字驱动；`Items` 缺省 0、`Driver` 小写、无记录消息对齐上游；9 个角色常量集中为唯一真相 + `AuthorizeAny` "任一角色" + 端点↔角色映射测试 + "403 先于 400"；并修 `ParseIso8601` 的**越界时间静默归一化**），`ctest -L phase6` **5 测试 / 997 断言**。剩余：GC 租约/幂等并发/tmp 名（C6.11~C6.13）、DMS/Delivery 收口（C6.7）、**1 GiB** 搬迁 RSS 绝对上限（C6.9）、远端仓储、组合根改接 SQLite 元数据仓储 |
...
P6 元数据语义完整化      🚧 进行中（切片 4/6：元数据仓储 + 校验和 + 12 步序列 + getFileList/角色）
P7 gRPC 适配层 + 双协议等价性
P8 认证授权与多租户
P9 硬化与交付
```
