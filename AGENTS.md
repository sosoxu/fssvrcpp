...
| 事实 | 现状 |
| --- | --- |
| **配置键三态** | **157** 个叶子键：**生效 141 / 拒绝启动 14 / 已读但无效果 2**（**`metadata.remote.*` 落地**后；B2a 新增 `deployment.expected_instances` 并让 `metadata.postgres.schema_version_check`、`deployment.clock_skew_tolerance_seconds` 转生效，B2b 让 `storage.posix.shared_mount_required` 转生效，E1b 让 `storage.posix.one_filesystem_per_partition` 转生效，ADR-006 让 `server.http.large_file_plane.*` 7 键转生效；**本轮让 `metadata.remote.{base_url,token_provider,static_token,timeout_ms}` 4 键转生效**：远端 Storage Service 元数据仓储 —— ⚠️ 该形态 `atomic_claim=false`（无 C1 原子领取），仅单实例、且 `leases.enabled` 必须为 false）。逐键见 [docs/operations.md](docs/operations.md) §1.3，由 `test_operations_doc` 自动比对（数字漂移还由该测试的跨文档一致性用例守着） |
| **PG 多实例（ADR-009）** | **`deployment.mode=multi` 已真的可运行**（B1）：组合根创建 PG 元数据/位置仓储 + PG 租约（`staging_leases`）+ `PgLeaderElection`（会话级 advisory lock，**专用锁连接**、只跑短语句），GC 的周期调度与 `POST /v2/gc:run` 由 leader 门控；任一创建/取锁失败或本构建无 libpq → exit 78。`.tmp.*` 已补齐 ADR-009 §4.5 的**随机后缀**；multi 下未配置 `instance_id`（默认 `local`）→ 自动生成唯一 id。**ADR-009 §4.2 的原子领取已交付（C1）**：`ClaimForWrite`/`MarkReady`/`ReleaseClaim` 三原语 + `claiming→ready`；用例先领取**再**复制（并发同一 `fileSource` **只复制一次**），`claiming` 期间第二个调用者有界等待 ~2s 后 **503 且不复制**；`state` 只做仓储列（不进 OSDU JSON），读路径只返回 `ready`。**ADR-009 §4.3 的在途租约与崩溃回收也已交付（C2）**：上传路径按 `file_source` 领取租约、复制/校验期间按 `renew_interval_seconds` 续租（续租失败**fail-closed 中止**）、**`MarkReady` 之后**才释放；GC 以"刚原子领取到的过期租约的 file_source 集合"驱动 `ReclaimStaleClaiming`，回收崩溃者留下的 `claiming` 行（阈值 `now - ttl_seconds`）。**C9.26 已交付（D）**：两个真实 `fss_server` 进程 + 共享 PG + **`kill -9`**，领导者在崩溃后转移、幸存者按租约回收崩溃者的 `claiming` 行 + 孤儿对象，同一 `fileSource` 重试 201（实测 8.0 s / 9.1 s ≈ TTL）。**仍未交付/未验证**：单实例（`leases.enabled=false`，默认）**没有**租约驱动的回收（无租约可证明死亡——按设计，已登记）、**NFS 语义（C9.27）—— C9.26 的"共享存储"是两个进程共享的**本地目录**，不是 NFS**。**B2a 已交付的生产就绪检查**：readiness 统一走 `shared_state_probe`（PG `SELECT 1` 探活 + `SELECT max(version) FROM schema_migrations` 比对；`metadata.postgres.schema_version_check` **生效**，版本不符 → **不 ready** 而非 exit 78），期望版本由**单一常量**并有一条从 `db/migrations/*.sql` 文件名反推的机械判据守着；C9.28 **PG 连接预算**（新键 `deployment.expected_instances`；按真实创建的池 + 锁连接计数，超限 **exit 78** 并列出数字与修法）；**PG↔本地时钟偏移**（`deployment.clock_skew_tolerance_seconds` **生效**，超容差 exit 78，测试用 `FSS_CLOCK_SKEW_INJECT_MS` 接缝双向钉住）。**B2b 已交付的共享性与滚动升级守卫**：每个实例写 `<root>/.fss_probe.<instance_id>`，并以 `instance_registry`（**10 s 心跳、30 s 存活窗口**）为协调点 —— **活着的对等实例的探针看不见 = 挂载没共享**（启动期 **exit 78**，运行期 → **不 ready**，原因含对等 id 与两条路径）；活对等实例的 `config_hash`（脱敏配置去掉身份/监听/URL 键后的 **SHA-256**）或 `service_version` 主次版本不一致 → **不 ready**（滚动升级守卫）；崩溃实例的行**过去只在启动期**清理 >300 s 的陈旧行。**ADR-009 §10 已全部收口**（多实例运维文档 → `docs/operations.md` §10 + `docs/runbook.md` §8.1~§8.6）。**E1a 已交付**：① `/v2/info` 暴露 `instanceId`（REST `instanceId` + gRPC `instance_id`，同源于 `UseCasePorts::instance_id`，**恒渲染**；multi 未配置时即自动生成的那个 id，与 `instance_registry` 行一致）；② `instance_registry` 陈旧行改为**启动期 + 运行期每 10 s tick** 的尽力而为清理（阈值仍 300 s；失败只告警、**不影响 readiness**；计数 `fss_instance_registry_{cleanup_runs,stale_rows_removed}_total` **只在 PG 模式存在**）。**仍未交付/未验证**：NFS 语义（C9.27，探针已交付、待生产环境执行，见 `phase9.md` §16）、跨实例 `createMetadata` 的 LB 粘性；以及真实跨主机 NTP 漂移、>2 实例、真实双版本滚动升级（版本分支由测试接缝驱动）、`app::ClockSkewGuard` 仍未被组合根装配；**E1a 的清理失败分支**（无 DELETE 权限 → 只告警）未用只读账号实测 |
...
| **sendfile 数据面（ADR-006）** | **下载面已交付**（`server.http.large_file_plane.enabled`，**默认 false**）：进程内第二个监听 socket，**复用控制面同一个已包装 handler**（鉴权/租户/错误体/指标同源），Range 用 L1 `fss::bytes` 自帧，字节走 `sendfile(2)`（无 fd 时回退用户态 64 KiB pump）；`/v2/info` 的 `largeFilePlane` 可见。**范围**：只 `GET/HEAD` 的 `/v1/transfer/{token}`（上传与 `splice` 不做）。**已知差异**：多段 Range 数据面降级 200（控制面 multipart 206）、方法 405 vs 404、超长 URI 414 vs 无响应。⚠️ **ADR-006 §6 第 5 条（真实存储上的受控基线复核）未完成**（本环境 WSL2+虚拟盘+loopback）⇒ 不得声称"门槛通过"；`BASELINE.tsv`/`docs/05` 未新增绝对数字 |
| **生产强校验** | `deployment.environment=production` 要求 `auth.mode=jwt` + 验签 + 密钥非空，否则**拒绝启动**（exit 78） |
| **未验证（环境受限）** | **真实 Storage Service 未联调**（remote 元数据形态的线协议由 mock 钉住）、真实存储/网卡/NVMe（含 **ADR-006 §6.5 的 sendfile 真实存储受控复核**）、NFS 语义（C9.27）、>5 GiB 分片、真实 S3/MinIO、K8s restricted、镜像 CVE 扫描、arm64、多实例 E2E（C9.26）、PG 连接预算（C9.28）、真实断电（ADR-008）、io_uring 收益（C9.29）—— 逐条依据见 [docs/phase-status.md](docs/phase-status.md) |
...
| **把"列表"压成单词再遍历**（`tr -d ' '` 处理 `(0 1 2)` → `012`） | 遍历到的元素根本不是你以为的元素。`run_sanitizers.sh` 因此把标签推成 `phase012`（不存在），"C1.7 全绿"其实是**空集合**：只有 phase0 跑过（P2-D07）。修法：按数字切词，并**断言每个标签匹配到的测试数 > 0**（与 C2.1 的"静态库 `link.txt` 是空证据"同类）。★ **同族第三个实例（本轮）**：`run_all_gates.sh` 只跑 `ctest -L phaseN`，而新增测试若只打`e1a`/`e1b`/`adr006` 这类**自定义标签、不带任何 `phaseN`**，则**门槛永远不跑它** —— 门槛全绿而新功能无人守。修法：新测试必须带对应阶段的 `phaseN` 标签，并用 `ctest -N -L phaseN` 确认它真的在集合里 |
...
| **把"每轮 `TryAcquire`"当成 failover** | 会话级 advisory lock 是**与会话同生共死**的：连接被切断（PG 重启/主备切换/网络抖动，**不是**本进程崩溃）后若只降级不重连，则**所有**实例永久答"我不是 leader" —— `pg_locks` 0 行、GC 全集群停摆，而 readiness 仍是 200（静默失败）。修法：`IsLeader()` 先"连接为空/不健康 ⇒ 重建（带冷却）"，再 `TryAcquire()`；且"选举不可用"要有独立日志/告警，不能与"别人是 leader"共用一句话（ADR-009 §4.4.1） |
...
| **在本机用容器搭 NFS + 多实例实验室**（含踩坑清单与处置） | `docs/lab-nfs-multiinstance.md` |
| **运维**：配置项全表（**157** 键逐条 + **三态清单**：生效/拒绝启动/已读但无效果）、可观测性、容量、安全 | `docs/operations.md` |
...
| ADR-006（大文件数据面） | **方案③已落地（下载面）**：进程内第二个监听 socket + `sendfile(2)`，**复用控制面同一个已包装 handler**（鉴权/租户/错误体/指标同源）、Range 用 L1 `fss::bytes` 自帧、可关闭（`server.http.large_file_plane.enabled`，**默认 false** ⇒ 默认仍走 httplib）；受控复核 2.12x 仍是方向依据。⚠️ **§6 第 5 条（真实存储受控基线复核）未完成** ⇒ 未达"门槛全通过" （`docs/adr/ADR-006-large-file-data-plane.md`） | 真实存储/网卡上的受控复核 <1.5x；或"复用控制面校验"做不到（只能复制一套 HTTP 语义）；或 TLS/mTLS 成为硬需求 |
