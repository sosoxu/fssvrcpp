...
P8 认证授权与多租户   🚧 进行中（切片 1/3：本地 JWT（HS256）+ 路由级预检 + 租户绑定 + 配置强校验）
P9 硬化与交付
```
