...
| 事实 | 现状 |
| --- | --- |
| **配置键三态** | **157** 个叶子键：**生效 137 / 拒绝启动 14 / 已读但无效果 6**（**ADR-006 落地**后；B2a 新增 `deployment.expected_instances` 并让 `metadata.postgres.schema_version_check`、`deployment.clock_skew_tolerance_seconds` 转生效，B2b 让 `storage.posix.shared_mount_required` 转生效，E1b 让 `storage.posix.one_filesystem_per_partition` 转生效，**本轮让 `server.http.large_file_plane.*` 7 键转生效**：下载面 switch/bind/port/use_sendfile/sendfile_chunk_bytes/workers/max_connections）。逐键见 [docs/operations.md](docs/operations.md) §1.3，由 `test_operations_doc` 自动比对（数字漂移还由该测试的跨文档一致性用例守着） |
...
| **未验证（环境受限）** | 真实存储/网卡/NVMe（含 **ADR-006 §6.5 的 sendfile 真实存储受控复核**）、NFS 语义（C9.27）、>5 GiB 分片、真实 S3/MinIO、K8s restricted、镜像 CVE 扫描、arm64、多实例 E2E（C9.26）、PG 连接预算（C9.28）、真实断电（ADR-008）、io_uring 收益（C9.29）—— 逐条依据见 [docs/phase-status.md](docs/phase-status.md) |
