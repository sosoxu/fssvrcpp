...
| **阶段 7（gRPC 适配层 + 双协议等价性）** | 🚧 **进行中（切片 1/3）**：契约 §5 的**唯一权威表**下沉 L3（`domain/contract/error_table.*`，HTTP/gRPC 两适配器从同一张表派生）+ `app::CallerFromHeaders` 抽出两条协议共用的调用方解析 + `fss_grpc_adapter` 接通（真实端口上实现 `GetInfo`/`Check`，其余 15 个 RPC 明确 `UNIMPLEMENTED`）+ C7.7 护栏（proto 头只能出现在 `adapters/grpc/`，含自证）；`ctest -L phase7` **2 测试 / 210 断言**（C7.2 已满足）。剩余：切片 2 = 其余 14 个一元 RPC + 契约 §6 等价性矩阵（C7.1/C7.3/C7.5）；切片 3 = 3 个流式/代理 RPC + C7.4/C7.6/C7.8/C7.10 |
...
P7 gRPC 适配层 + 双协议等价性   🚧 进行中（切片 1/3：代码生成 + 错误表 + 运维 RPC + proto 护栏）
P8 认证授权与多租户
P9 硬化与交付
```
