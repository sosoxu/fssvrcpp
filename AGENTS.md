...
| 阶段 4 后续（**已在阶段 10 接入**） | 组合根接 `config/fss.example.json`：**阶段 10 切片 1 已实现**（CLI > env > file > 默认；`--config`/`--set`/`--print-config`）。156 个叶子键中 **66 个已接通 / 90 个未接通**（逐键见 `docs/operations.md` §1.3/§1.4）；生产环境有"必须 jwt + 必须验签 + 密钥非空"的启动强校验 |
...
| **阶段 9（硬化与交付）** | ✅ **已完成并通过门槛**（10 个切片要点，C9.1~C9.13/C9.15/C9.16/C9.25 全部满足）：① **C9.1 并发**（100 并发 JSON 零 5xx；8 并发大文件逐路 SHA-256 + 存储侧 checksum 三方一致）；② **C9.2 故障注入 + 恢复 ≤30 秒**（503/500/502 + 非致命依赖；`mock_entitlements --fail-file` 注入与恢复）；③ **C9.3 六类资源上限**各一条"被拒绝" + 对照；④ **C9.6 指标**（`/metrics` 格式合法、值真的动过、不含 secret；**真实进程**已接入存储计量 —— 见 P9-D10）；⑤ **C9.5/C9.25 GC 清理**（`IBlobStore::remove_temp_files`：够旧删、在途保护且计数可观测、被位置记录"引用"也照删）；⑥ **C9.4/C9.11/C9.13/C9.15 容量基线**（独立进程 + 互不重叠绑核 + 每点位 3 次取中位数；`bench/capacity_bench.cpp` + `scripts/bench_baseline.sh` + `docs/appendix/capacity-baseline/BASELINE.tsv`）；⑦ **C9.12/C9.16 ADR-006 定稿**（同一负载生成器受控复核 `sendfile` vs httplib = **2.12x / 2.02x ≥ 1.5x → 采纳方向**，实现未交付）；⑧ **C9.7 门槛纳入 P9**（P0~P9 顺序全绿，**总耗时 225 s / 10 阶段**）；⑨ **C9.8 容器镜像**（多阶段 `Dockerfile` + `scripts/verify_image.sh`：readiness 200、强制 jwt、非 root、HEALTHCHECK **全部实测**）；⑩ **C9.9 运维文档 + C9.10 风险收口**（`operations.md`/`runbook.md` + 自动比对测试覆盖 156 个配置键；§16.1 覆盖全部 20 行风险）。修掉 **P9-D01~D10**（测试替身非线程安全；POSIX/S3 时间戳语义；数据面错误码被覆盖；流式 handler 不读体；护栏白名单；**组合根未接指标注册表与日志脱敏**）。`ctest -L phase9` **6 测试 / 466 断言**；`ctest` **75/75**。**未做/未验证（登记）**：组合根当时未接 `config/fss.example.json`（156 键中 125 个未接通 —— **阶段 10 切片 1 已把接通数提到 66**，剩 90，逐键见 `operations.md` §1.3）、PG 仓储/租约与 `mode=multi` 运行形态、sendfile 数据面实现、GC 调度与端点、真实硬件/多进程/容器类判据（C9.14/C9.17~C9.22/C9.24/C9.26~C9.30） |
| **阶段 10（配置面接线）** | 🚧 **进行中（切片 1/2 完成）**：让 `config/fss.example.json`（带注释 JSON）成为**真配置源**，优先级 **CLI > 环境变量 > 配置文件 > 默认值**（`fss::config::Load` 复用，未重写加载器）。① **C10.1** `--config`/`FSS_CONFIG` + `--set <k>=<v>` + `--print-config`（脱敏 + 逐键来源）；配置非法 → `ProblemsToString()` + **exit 78**；② **C10.2** 优先级与来源在横幅逐键可见（含"通用名 vs 旧别名"歧义告警）；③ **C10.3/C10.4/C10.6** `server.http.*`（含 `ValidateOptions` 前置校验）、`storage.*`（含 `io_engine` 探测/回退、`instance_id` 进临时名）、`observability.*`（日志级别/格式/脱敏、审计开关、`/metrics` 开关与路径）全部作用到真实进程；④ **C10.5 生产强校验**：`deployment.environment=production` 时 `auth.mode` 必须 `jwt`、必须验签、密钥非空 —— 否则**拒绝启动**（把 P9 登记的"默认无鉴权"变成不可误配）；⑤ **C10.7** `docs/operations.md` 的接通状态与实现一致：**156 键 → 66 接通 / 90 未接通**（P9 时是 31/125）；⑥ **C10.8 自证**：把 `load_request.file_path` 置空（退回只读环境变量）→ `test_config_wiring` **8 个用例失败**，还原后全绿。新测试 `tests/integration/test_config_wiring.cpp`（**10 用例 / 137 断言**）+ `scripts/verify_config_wiring.sh`（22 条断言）；`run_all_gates.sh` 纳入 P10（**P0~P10 全绿，230 s / 11 阶段**）；`ctest` **76/76**。**未做/降级（如实登记）**：`observability.audit_fail_closed` 已读但**行为未实现**（无"致命审计"路径）；`gc.*`/`leases.*`/`leader_election.*`/`large_file_plane.*`/`proxy_mode` 等 **90 键未接通**；`storage.posix.durability=never` 只能经旧别名（schema enum 不含）；`io_engine=uring` 即使内核可用也拒绝（ADR-010 U1~U4） |
...
P10 配置面接线   🚧 进行中（切片 1/2：--config/--set/--print-config + 优先级与来源可见 + server.http/storage/auth/observability 接通 + 生产强校验 + 156 键 66 接通/90 未接通 + 新测试 10 用例/137 断言）
```
...
| **运维**：配置项全表（156 键逐条 + 未接通清单）、可观测性、容量、安全 | `docs/operations.md` |
