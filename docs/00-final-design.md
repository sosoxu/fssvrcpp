...
P2 领域模型 + 13 个端口 + 应用层纯逻辑
P3 集中存储驱动 + 位置仓储 + 数据面
P4 REST 适配层 + 端到端垂直切片（POSIX）   ← 此阶段服务可被 OSDU 客户端真实使用
P5 对象存储驱动（S3 SigV4 + mock-S3 独立验签）
P6 元数据记录语义完整化（12 步序列 + 回滚 + 版本链 + DMS + Delivery）
P7 gRPC 适配层 + 双协议等价性            ← 此阶段"双协议"达成
P8 认证授权与多租户
