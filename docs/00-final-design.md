...
| [**ADR-006**](adr/ADR-006-large-file-data-plane.md) | 大文件数据面：**采纳 sendfile 方向**（受控复核 2.12x ≥ 1.5x）；**下载面已交付**（进程内第二个监听 socket；复用控制面**同一个**已包装 handler；`enabled=false` 默认关闭）。⚠️ ADR-006 §6 第 5 条（真实存储/网卡受控复核）**仍未完成** | **已交付（下载面）/ §6 第 5 条未完成** |
...
### 5.t 本轮更正的既有结论（B1：`deployment.mode=multi` 从"拒绝启动"到"真的装配 PG 运行形态"）

| # | 旧结论（记录于） | 现状（依据） | 影响 |
| --- | --- | --- | --- |
| 1 | 「`deployment.mode=multi` **拒绝启动**：需要 PG 仓储 + PG 租约 + 数据库时钟（ADR-009），本版本尚未交付」（`src/main/server_main.cpp`、`docs/operations.md` §1.3/§7.3、`docs/runbook.md` §8、`docs/02-design.md` 的 R-26） | **推翻（本轮 / B1）**：A1/A2 的三件 PG 数据层（`§15`/`§16`）本切片接进组合根——`metadata.repository=postgres`/`location.repository=postgres` 创建 PG 仓储、`leases.enabled=true` 创建 `PostgresLeaseRepository`（`staging_leases`）、`leader_election.enabled=true` 创建新增 L2 `PgLeaderElection`（**专用锁连接** + `pg_try_advisory_lock`，`IsLeader()` 每轮**验证连接健康**）；GC 的周期调度与 `POST /v2/gc:run` 由 leader 门控。任一创建/取锁失败或无 libpq → **exit 78**（可执行修复指令），绝不回退 SQLite | ① `location.postgres.*`/`metadata.postgres.*`（5 键）、`leases.enabled`、`leader_election.*`（3 键）共 **9 键**移入「生效」→ 三态 **113/18/25 → 122/16/18**（`docs/operations.md` §1.3；`test_operations_doc` 机械断言）；② `deployment.instance_id` 在 multi 下未配置/为空时**自动生成**（默认 `local` 会让多实例共享标识 → M1 静默串数据）；③ POSIX `.tmp.*` 补齐 ADR-009 §4.5 的**随机后缀**；④ 证据 `docs/test-evidence/phase10.md` §17（两引擎 + 4 条 R1 注入）；⑤ **仍未交付**：共享挂载探针、readiness PG 探活+迁移版本校验、崩溃注入/完整多实例 E2E（C9.26）、NFS 语义（C9.27）、`/v2/info` 的 `instanceId` |

---

### 5.aa 本轮更正的既有结论（E1b：`storage.posix.one_filesystem_per_partition` 从「已读但无效果」到「启动期强制校验」）

| # | 旧结论（记录于） | 现状（依据） | 影响 |
| --- | --- | --- | --- |
| 1 | 「`storage.posix.one_filesystem_per_partition` 是最后一个「已读但无效果」的键：组合根未接线，改它对真实进程**没有效果**，分盘只能靠部署手段」（`src/common/config/core_schema.cpp:149`、`config/fss.example.json:136`、`docs/operations.md` §1.2/§1.3.3、`docs/02-design.md` R-28、`docs/runbook.md` §8.1） | **推翻（本轮 / E1b）**：它不是"没有语义"，缺的是**实现**。新增 L2 `src/infra/blob/posix/partition_filesystem_check.{h,cpp}`（与 `shared_mount_probe.*` 同层同风格）：**只读** `::stat(2)`（跟随符号链接，`std::filesystem` 没有 `st_dev` API，precedent `posix_blob_store.cpp:9,64-69`），启动期验证部署断言——**规则 A**：每个 partition 的 staging/persistent 目录（= `<storage.posix.root>/blobs/<container>`，容器名走**同一个** `ObjectKeyPolicy::ContainerFor`）必须与 `storage.posix.root` **分盘**；**规则 B**：两个不同 partition 不得解析到**同一个**文件系统。组合根在 `StaticPartitionRegistry` 之后（`src/main/server_main.cpp`）调用，违规 → **exit 78**，消息含键名/分区名/完整路径/两组 `st_dev`/`syncfs` 理由/修法；`storage.driver=s3` + `true` → **拒绝启动**（s3 无 partition 文件系统可隔离）；横幅新增 `part fs check :` 打印已校验设备号（R11） | ① 三态 **129/15/13 → 130/15/12**（键数仍 157；`docs/operations.md` §1.3，`test_operations_doc` 机械断言）；② `docs/operations.md` §1.3.3 的「已读但无效果」清单里 **`storage` 前缀已清空**（该清单**仍有 12 个键**，全部是"能力未实现"类：`server.http.large_file_plane.*` 6 + `metadata.remote.*` 4 + 两个 `*.sqlite.max_write_concurrency`，见该节；**不要**把它读成"清单已清空"）；③ `false`（默认）时**不 stat、不拒绝**，行为与接线前一致；④ **规则 B 在当前生产形态下是空集**（组合根只由 `partition.file.opendes.*` 构造**一个** partition）——它仍被实现并由**合成多 partition 输入**的 L2 用例（U3）钉住；⑤ **刻意不检查**同一 partition 的 staging 与 persistent 是否同盘（`docs/operations.md` §10.3 的布局表允许一个 partition 用两个独立卷；加这条规则会误判合法部署，U5 钉住"没有加"）；⑥ **未验证（不声称）**：该文件系统上是否还有**非本服务**负载——进程内不可观测，属部署纪律；⑦ **C9.24（`syncfs` 跨实例干扰量级）仍未被测量**：本切片只把"按 partition 分盘"变成可被拒绝启动的断言，**不等于**量级已知；⑧ 证据：`tests/integration/test_partition_filesystem_check.cpp`（U1~U6 + W1~W5）、`docs/test-evidence/phase10.md` §24（含 I1~I5 与护栏注入的自证） |

### 5.bb 本轮更正的既有结论（ADR-006：大文件数据面从"只采纳方向"到"落地 sendfile 下载面"）

| # | 旧结论（记录于） | 现状（依据） | 影响 |
| --- | --- | --- | --- |
| 1 | 「**sendfile 数据面**：只采纳方向（受控复核 2.12x ≥ 1.5x），**实现未交付**；`server.http.large_file_plane.enabled=true` → **exit 78**；默认仍走 httplib」（`docs/adr/ADR-006-large-file-data-plane.md` 状态头、`AGENTS.md` §0.1、`docs/operations.md` §1.2/§1.3.2、`docs/02-design.md` §7.7/R-14、`docs/04-implementation-plan.md` 的「下一步」第 5 条） | **推翻（本轮）**：数据面**已交付**且默认仍关闭。按 ADR-006 的**方案 ③**（进程内自持 socket）落地：新增 L1 `src/common/http/large_file_plane.{h,cpp}`（第二个监听 socket + `sendfile(2)`），L1 `bytes::ByteSource::NativeFd()`（可选零拷贝能力，默认 `-1` = 必须回退用户态），L2 `src/infra/transfer/posix_file_byte_source.{h,cpp}`（`pread` + 自有偏移 + EINTR + RAII `::close`，`Seek` 边界与 `BlobByteSource` 逐条一致），端口默认方法 `IBlobStore::OpenNativeRead`（**只有 POSIX 覆盖**，内存/S3 得 `kUnimplemented`），`MeteredBlobStore` 转发；组合根 `transfers.open_get` 优先取原生源 | ① 7 个 `server.http.large_file_plane.*` 键移入「生效」→ 三态 **130/15/12 → 137/14/6**（`docs/operations.md` §1.3，`test_operations_doc` 机械断言）；② `/v2/info` 新增 `largeFilePlane`（`disabled`\|`sendfile`\|`userspace`，REST + proto 字段 **13**，R11）；③ 启动横幅新增 `plane bind :`（bind:port / workers / max_conn / sendfile on-off）；④ 启动失败（端口被占等）→ **exit 78 + 可读修法**，绝不静默禁用；⑤ `enabled=false`（默认）时**不建监听、不启动线程**，行为与接线前逐字一致（P9）；⑥ 证据：`tests/integration/test_large_file_plane.cpp`（P1~P12，**13 用例 / 648 断言**，`ctest -L adr006` 全绿）+ `docs/test-evidence/phase10.md` §25 |
| 2 | 隐含前提：「数据面必须把字节面从 HTTP 适配层剥离（独立进程/端口）」以及「控制面校验会被复制一份」（ADR-006 §2 方案②的顾虑；§4 第 1 条） | **按 §4 第 1 条消解**：数据面**不实现任何 HTTP 语义**——它把请求解析成 `fss::http::Request` 后调用 `Router::BuildTransferGetHandler()` 返回的**同一个已包装 handler**（鉴权/租户/`ErrorToResponse`/`metrics_.Observe` 都是那份代码）；解析阶段的错误体调**同一份** `fss::http::ErrorBody`，访问日志调**同一份** `fss::http::LogAccess`（字段集由 P11 逐字段比对）。R1 注入 I1（让数据面绕过该 handler 直接调 `open_get`）→ **P3 失败**（401/403/404 变成 200），证明判据能抓住"第二套语义" | 方案②（独立进程 + 复制校验）**不再需要**；ADR-006 §4 第 1 条从"建议"变成**结构上必然**（P11 的字段集等价 + P3 的鉴权重跑 + P8 的逐字段矩阵） |
| 3 | 「`MeteredBlobStore` 的 `get()` 是下载字节的唯一计数点」（P9/C9.6 的隐含前提） | **部分推翻 / 计数点分裂**：`sendfile` **绕过** `ByteSource::Read` 与 `get()`。新的两条互补规则（在 `metered_blob_store.*` 与数据面里都有注释）：① **用户态拷贝**（控制面 `StreamPump` 或 `use_sendfile=false`）由 `MeteredBlobStore::OpenNativeRead` 返回的 `MeteredNativeSource` 在 `Read` 上记字节、析构时记一次 `op=get`；② **`sendfile`** 由数据面在服务完成后调 `MeteredBlobStore::RecordNativeRead(bytes, ok)` 记**同样的** `fss_storage_bytes_total{direction="out"}` 与 `fss_storage_operations_total{driver,op="get",outcome}`。二者互斥，**绝不重复计数**（P5 双向断言：控制面下载只动存储计数、数据面下载同时动数据面 `sendfile` 计数与存储计数） | 若不做这一步，控制面下载的 `direction="out"` 会**静默归零**（实测发现并修掉：早期实现只让数据面补记 ⇒ 控制面下载不再计数）。运维判据不变（族名/标签与 `get()` 逐字相同），但"`op=get` 的次数"从"每个用户态读块一次"变成"每次打开一次" |
| 4 | （范围假设）「数据面 = 数据面，方法/多段都应与控制面等价」（ADR-006 §6.1 的字面读法） | **三处**必须显式登记，不能静默：① **只下载**——数据面只认 `GET/HEAD`；`PUT` 留在控制面（`sendfile` 服务不了上传，`splice`/`receive-file` 不在范围），其余方法 → **405 + `Allow: GET, HEAD`**（控制面因注册了 `PUT` 而是 404/200）；② **多段 Range 降级**——数据面对 `bytes=a-b,c-d` 返回 **200 全量**，控制面对可寻址来源返回 **multipart 206**（ADR-006 §6.1 自己叫"多段降级"；P2/P8 双向断言这个**唯一**被文档化的差异）；③ **超长 URI**——数据面回 **414**，而控制面因 httplib 的 header reader 在 URI 检查**之前**失败而**不返回任何 HTTP 响应**（`docs/00-final-design.md` §5.x 第 1 条已记录库行为；P4 显式断言两侧的**实际**行为，不声称"同状态"） | 不能把"数据面等价"写成无限定的全称（R7）；ADR-006 §6.1 的 `413` 条目是 **PUT-only** 的关注点（数据面不服务上传 ⇒ 不适用），已在 ADR-006 与证据里登记 |
| 5 | （ADR-006 §6.5 的隐含前提）「受控基线复核可以在本环境完成」 | **明确做不到**：§6.5 要求在**真实存储（NVMe/NFS）+ 真实网卡**上重测并更新 `docs/appendix/capacity-baseline/BASELINE.tsv` 与 `docs/05`。本环境是 **WSL2 + 虚拟盘 + loopback**，且 ADR-006 §5 自己声明这些绝对值**无结论**。因此**不新增任何绝对数字**，只把脚本 `scripts/bench_sendfile_ab.sh` 与执行前置条件**移交生产环境**（与 C9.27 探针同一处置）；本机只报告"页缓存/loopback"条件下的**比值与自用吞吐**，明确标注不迁移 | §6 **第 5 条保持未勾选**（唯一一条）；新增未验证项：真实存储/网卡基线、**TLS/mTLS**（ADR-006 §7.3 的重开条件）、真实跨主机/多实例数据面、**>2 GiB 对象**（本切片实测最大 **1 GiB**）、`splice`/上传加速（**刻意未做**）。`BASELINE.tsv` 与 `docs/05` **不新增绝对数字** |

### 5.cc 本轮更正的既有结论（`metadata.repository=remote`：远端 Storage Service 元数据仓储从"未交付"到"落地"）

> 本切片 = `metadata.remote.{base_url,token_provider,static_token,timeout_ms}` 4 个键转生效；
> 三态 **137/14/6 → 141/14/2**（键数仍 157）。决策文：`docs/adr/ADR-004-persistence-strategy.md`
> （状态头已更新 + 文末「本切片的交付与偏离」）；证据：`docs/test-evidence/phase10.md` §26。

| # | 旧结论（记录于） | 现状（依据） | 影响 |
| --- | --- | --- | --- |
| 1 | 「`metadata.repository=remote` **未交付** → exit 78（组合根显式拒绝，绝不静默降级）」；`metadata.remote.*` 4 键 =「已读但无效果（远端元数据仓储未交付）」（`docs/operations.md` §1.2.7/§1.3.3、`AGENTS.md` §0.1、`docs/04-implementation-plan.md` 的「下一步」、`docs/02-design.md` 风险行） | **推翻**：新增 L2 `src/infra/metadata/remote/remote_metadata_repository.{h,cpp}`（libcurl；`PUT {base}/records` / `GET {base}/records/{id}` / `POST {base}/records/{id}:delete` **必须 204**；`base_url` 是**基址**，追加路径），组合根四条 fail-closed 前置（`deployment.mode=single` + `base_url` 非空 + `token_provider=static` + `leases.enabled=false`，任一不满足 → **exit 78 + 点名键与修法**），`ports.shared_state_probe` 接入就绪探针（`GET {base}/records/{固定不存在 id}`，**404 = 服务活着**） | ① 4 键 → **生效**；三态 **137/14/6 → 141/14/2**（`test_operations_doc` 机械断言）；② 新指标族 `fss_metadata_remote_requests_total{op,outcome}`（**只在 remote 形态注册**；M12 断言 sqlite 形态没有它）；③ 启动横幅 `repositories : metadata=remote（… atomic_claim=false）`（R11）；④ 依赖挂 → readiness **503 且 liveness 仍 200**（M10）；⑤ 证据：`tests/integration/test_remote_metadata_repository.cpp`（M1~M12 + M8b，**13 用例**）+ `mock_validators.py --mode storage` + `tests/framework/mock_storage.h` |
| 2 | ADR-004 的隐含前提：「`IMetadataRepository` 的语义被设计成**同时满足两种后端**」（ADR-004 §决策；`ports.h` 只列 3 方法） | **部分推翻**：端口实际已含 ADR-009 §4.2 的 C1 原子领取（`ClaimForWrite`/`MarkReady`/`ReleaseClaim`/`ReclaimStaleClaiming`）与 `state`/`is_latest` **仓储列**；远端 Storage Service **既无条件插入原语、也无 `state` 列** ⇒ 无法诚实实现。做法：端口新增**defaulted** `MetadataCapabilities{atomic_claim, backend_name}`（与 `IBlobStore::OpenNativeRead` 同一纪律），只有 remote 覆盖成 `{false,"remote"}`；四个领取原语返回 `kUnimplemented`（点名能力与理由）；用例 `CreateFileMetadata` 按 `capabilities().atomic_claim` 分支走**无领取路径** | ⚠️ **有意的降级**：remote 形态下并发同一 `fileSource` 的创建**没有互斥**，可能复制两次并产生**两条版本**（对象物理位置相同 ⇒ 不丢数据）。因此组合根强制 single + 无租约。这**不是 bug**，是"生产用 PG（multi 强制 postgres）、remote 仅单实例"的代价。已写进 `docs/operations.md` §1.3.3、`docs/runbook.md` §2.1、ADR-004 与本表 |
...
P2 领域模型 + 15 个端口 + 应用层纯逻辑     ✅ 已完成（10 测试 / 996 断言；C2.10 的跨实现复用已在 P3/P5/P6 闭合）
...
| # | 项 | 门槛 |
| --- | --- | --- |
| 5 | ~~**ADR-006 的受控复核**~~（`sendfile` 数据面 vs httplib，同一负载生成器；≥1.5x 才定稿，否则放弃） | ✅ **受控复核已完成（P9）：2.12x ≥ 1.5x → 采纳方向**；✅ **下载面已交付（本轮）**，见 `docs/adr/ADR-006-large-file-data-plane.md` 与 §5.bb。⚠️ **仍未完成**：ADR-006 §6 第 5 条（**真实存储/网卡**上的受控基线复核 → 更新 `BASELINE.tsv`/`docs/05`）；本环境是 WSL2 + 虚拟盘 + loopback，绝对值无结论 ⇒ 只移交脚本 `scripts/bench_sendfile_ab.sh` 给生产环境，不新增绝对数字 |
