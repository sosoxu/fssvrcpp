...
| **P9** | 硬化与交付 | 并发/容量基线（独立负载进程）、故障注入、指标、GC、打包、部署模板、运维手册；**定稿 ADR-006** | `ctest -L phase9 && scripts/run_all_gates.sh` | 🚧 **进行中（切片 1/3 完成 + 切片 2 的指标/GC 部分；C9.1~C9.3、C9.5、C9.6、C9.25 满足）** |
...
> **当前进度（切片 1/3 + 切片 2 的指标/GC 部分）**：`ctest -L phase9` **5 测试 / 451 断言**
> （C9.1/C9.2/C9.3/C9.5/C9.6/C9.25 满足）。
...
> "某条位置记录恰好指向它"而放过；⑦ 顺带修掉 **5 个"看起来通过"型缺陷**（P9-D04~D08：
> 时间戳时基/格式、临时文件计数翻倍、数据面错误码被"读体失败 400"覆盖、流式 handler 不读体却回 2xx）。
> **剩余**：切片 2 的性能基线部分（C9.4/C9.11/C9.13/C9.15）+ ADR-006 复核（C9.12/C9.16）；
> 切片 3 = 门槛纳入 P9（C9.7）+ 镜像（C9.8）+ 运维文档（C9.9）
> + 风险收口（C9.10）。需真实硬件/多进程/容器的判据将如实登记为未验证。
