...
| **P10** | 配置面接线 | `--config`/`--set`/`--print-config` + `fss::config::Load` 接入组合根、旧环境变量别名兼容、生产强校验、`docs/operations.md` 接通状态逐键更新 | `ctest -L phase10 && scripts/verify_config_wiring.sh` | ✅ **切片 1/2/3/4/5/6a/6b 全部完成（C10.1~C10.20）**：三态 157 键 **生效 130 / 拒绝启动 15 / 已读但无效果 12**（`docs/operations.md` §1.3；B2a/B2b/E1b 后；ADR-008 的 P4 +1、**C10.20** 的 6 键、**E1b** 的 1 键）；真实二进制证据见 `docs/test-evidence/phase10.md`。**期间补交 P9 的 C9.31**（GC 按需端点；不改任何配置键，三态计数不变） |
...
> **当前进度（切片 1/2/3 全部完成 + P9 补交 C9.30/C9.31/C9.32）**：`ctest -L phase9`
> **10 测试 / 885 断言**（C9.30 第三半补齐后 +147）；
> `run_all_gates.sh` **P0~P9 全绿**（P10 已并入）；`ctest` 84/84。
> C9.1~C9.13、C9.15、C9.16、C9.25、**C9.31**（P9 补交）、**C9.32**（P9 补交）、
> **C9.30**（P9 补交，P10 期间完成第三半）满足（C9.14/C9.17~C9.22/C9.24/C9.26~C9.29 为环境不具备的
> 判据，已如实登记为未验证，见证据 §11）。
...
| **C9.26** | **多实例端到端**：2 个真实进程 + 共享 PG + 共享目录，跑完整 上传→登记→下载→删除 流程，并注入实例崩溃（验证 `claiming` 记录的租约回收） —— ✅ **已交付**（`tests/integration/test_multi_crash_recovery.cpp`，见 `test-evidence/phase10.md` §20）：2 个真实进程 + 共享 PG + **本地共享目录** + 真实 `kill -9` + 幸存者按租约回收 `claiming` 行 + 同 `fileSource` 重试 201。★ **共享存储是本地目录被两个进程共用，不是 NFS** ⇒ **C9.27 仍未验证**；共享挂载探针仍未做（见 `test-evidence/phase10.md` §17.6）。**口径差异**：本条要的是"多进程 + 共享 PG + 共享目录 + 崩溃注入"，与容器的只读 rootfs / 最小权限 / 资源上限 / 健康检查**不是同一件事**（后者见 `docs/test-evidence/phase9-image.md` §10） |
...
| **C9.30** | `/v2/info` 与指标正确暴露 `ioEngine` / `ioUringAvailable`；在**不允许 io_uring 的部署**里所有 OSDU 端点行为不变 —— ✅ **已完整满足（三半逐条给证据）**：① 「不允许 io_uring 的部署里 OSDU 端点行为不变」**已在 Docker 上实测**（Docker 默认 seccomp：容器内 `Seccomp: 2`、io_uring 被阻断（ADR-010 实测 `EPERM`；本轮另用 `python3` 直接 `syscall` 双向复验：宿主 `rc>=0` / 默认 seccomp 容器 `EPERM`）→ readiness **200** + 上传/读回 SHA-256 一致；命令见 `docs/test-evidence/phase9-image.md` §10 H8）；② 「**指标**暴露」**已实测**：真实进程 `/metrics` 有 `fss_io_engine{engine="blocking",requested="blocking"} 1` **且**新增 `fss_io_uring_available 0\|1`（宿主能力探测，与 `ioEngine` 分列；`docs/test-evidence/phase9.md` 的 C9.30 节 ③）；③ 「**`/v2/info`** 暴露 `ioEngine`/`ioUringAvailable`」**已交付**：REST 追加 `"ioEngine":"blocking"` + `"ioUringAvailable":<bool>`（camelCase 扩展字段，见契约 §2.12/§4.2），gRPC `InfoResponse` 追加字段号 **10/11**（`json_name` 对齐，C7.5），两条协议**同源**（都读 `app::GetInfo` 的 `VersionInfo` ⇒ C7.3 按构造保证）；判据 `tests/integration/test_io_engine_exposure.cpp`（7 用例 / 147 断言）。★ **语义**：`ioUringAvailable` 只表达**宿主能力**（`true` ≠ 已启用），`ioEngine` 当前恒为 `blocking`（ADR-010 U1~U4）。★ **区分力**来自探测注入接缝 `FSS_IO_PROBE_INJECT=available\|blocked`（环境变量，**不是配置键**，三态 107/18/31 不变）：两方向都断言 ⇒ 字段既不是恒 `false` 也不是恒 `true`；注入下 `ioEngine` 仍 `blocking`、`storage.io_engine=uring` 仍 **exit 78**。R1 自证三注入（REST 硬编码 false / gRPC 写死 false / 删指标）各让对应用例失败，输出见证据 |
...
| **C9.32** | **（P9 补交，P10 期间完成）未预期异常必须以干净退出码结束**：`main()` 顶层兜底（函数体搬进 `RunServer`，`main` = 薄包装 + `catch (const std::exception&)` + `catch (...)`）→ 退出码 **70（EX_SOFTWARE）** + 可读原因（含"容器 pids 不足"的提示），**不得出现 `terminate`/139**；既有退出码**逐字不变**（配置非法 78、未知 CLI 参数 2）。故障注入接缝 = 环境变量 `FSS_STARTUP_FAULT_INJECT`（`throw_system_error` / `throw_bad_alloc` / `throw_unknown` / `throw_after_start`；**不是配置键**，三态保持 107/18/31）。容器 `--pids-limit=64` 的**真实回归断言**（`ExitCode==70` + stderr 含「未预期异常」+ 无 `terminate called`，另有 pids=66 正控）见 `scripts/verify_image.sh` 的 H4a/H4a2 与 `docs/test-evidence/phase9-image.md` §10.12；进程内判据见 `tests/integration/test_startup_faults.cpp`；R1 自证：①去掉顶层 catch（→ exit 134/139）、②`kExitInternalError` 改成 0、③删掉 `catch (...)`（→ 非 std 类型逃出 → terminate）三种错误实现必须让对应用例**失败** |

**退出条件**：C9.1–C9.10 满足（含 P9 补交的 **C9.31**、**C9.32**），证据写入 `docs/test-evidence/phase9.md`（容器侧另有 `docs/test-evidence/phase9-image.md`）。
...
三态（E1b 后的当前值）：**生效 130 / 拒绝启动 15 / 已读但无效果 12 = 157**（`docs/operations.md` §1.3；B2a/B2b/E1b 的净变化见该节末尾）。
...
* **C10.11**：16 个未实现能力的非默认值 → **exit 78 +「未实现 + 下一步」**；
  `docs/operations.md` 逐键三态化（**当前为 生效 130 / 拒绝启动 15 / 已读但无效果 12 = 157**；各切片的历史计数与理由见 `docs/test-evidence/phase10.md`）。
...
* 三态计数（E1b 后的当前值）：**生效 130 / 拒绝启动 15 / 已读但无效果 12 = 157**（`operations.md` §1.3 + `test_operations_doc` 机械断言）。
