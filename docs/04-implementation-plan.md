...
| **P9** | 硬化与交付 | 并发/容量基线（独立负载进程）、故障注入、指标、GC、打包、部署模板、运维手册；**定稿 ADR-006** | `ctest -L phase9 && scripts/run_all_gates.sh` | 🚧 **进行中（切片 1/3：并发 + 故障注入 + 资源上限；C9.1~C9.3 满足）** |
...
> **当前进度（切片 1/3）**：`ctest -L phase9` **3 测试 / 215 断言**（C9.1/C9.2/C9.3 满足）。
...
> **剩余**：切片 2 = 指标（C9.6）+ GC 清理（C9.5/C9.25）+ 性能基线（C9.4/C9.11/C9.13/C9.15）
> + ADR-006 复核（C9.12/C9.16）；切片 3 = 门槛纳入 P9（C9.7）+ 镜像（C9.8）+ 运维文档（C9.9）
