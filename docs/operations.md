...
>
> 三态计数见 §1.3（**生效 93 / 拒绝启动 21 / 已读但无效果 42；合计 156**，与 `config/fss.example.json` 的叶子键一一对应）。
...
> 计数由本节的三个清单逐条相加得出：**93 + 21 + 42 = 156**。

| 状态 | 键数 | 说明 |
| --- | --- | --- |
| **生效** | **93** | 读取后真的改变运行行为（含"非法值拒绝启动"的触发条件，写在 §1.2 对应行） |
| **拒绝启动（触发条件）** | **21** | 非默认/不支持的值 → **exit 78（EX_CONFIG）** + 「未实现 + 下一步」 |
| **已读但无效果** | **42** | 组合根未接线（或读了但行为未实现）→ 改它对真实进程没有影响；理由与下一步逐个登记 |
...
| 项 | 状态 | 说明 / 证据 |
| --- | --- | --- |
| 组合根接 `config/fss.example.json` | **切片 1/2/3/4 已接通（含 C10.16）** | `--config`/`FSS_CONFIG` + `--set` + 环境变量；156 键的三态计数为 **生效 93 / 拒绝启动 21 / 已读但无效果 42**（逐键见 §1.3；样例配置在 C10.10 用例里真的启动成功）。切片 3 新接通 9 键：`observability.audit_fail_closed`、`metadata.sqlite.busy_timeout_ms`、`location.sqlite.{busy_timeout_ms,journal_mode}`、`auth.jwt.roles_claim`、`auth.local_roles.*`（3）、`server.grpc.enabled`；C10.16 新增：`partition.file.opendes.max_file_bytes` → 生效，`partition.file.opendes.{allowed,default}_checksum_algorithm` → 拒绝启动。**C10.16 续**：`storage.posix.{atomic_write,dir_mode,file_mode,fadvise_random,fadvise_dontneed_after_large_read}` 与 `partition.file.opendes.{staging,persistent}_container` → 生效（+7）；`storage.posix.{group_commit_max_batch,sync_dir_after_batch}`（ADR-008 的 P4 未实现）与 `partition.file.opendes.storage_driver`（驱动冲突）→ 拒绝启动（+3）。**切片 4（本轮）**：`server.http.transfer_max_body_bytes`（数据面 PUT 上限 = 全局键与 `partition.file.<p>.max_file_bytes` 取较小者）、`metadata.sqlite.{journal_mode,synchronous}`、`location.sqlite.synchronous` → 生效（+4） |
...
| `storage.posix.{atomic_write,dir_mode,file_mode,fadvise_random,fadvise_dontneed_after_large_read,group_commit_max_batch,sync_dir_after_batch}` 与 `partition.file.opendes.{staging_container,persistent_container,storage_driver}` | **已定案（C10.16 续）** | `PosixBlobStoreOptions` 已加 5 个真实字段（atomic_write/dir_mode/file_mode/fadvise_*）并接通；`PartitionConfig` 已加容器名/分区驱动字段：容器名接通、分区驱动冲突 → 拒绝启动；`group_commit_max_batch`/`sync_dir_after_batch` 因 ADR-008 的 P4 未实现 → 拒绝启动（§1.3.2） |
