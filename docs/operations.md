...
>
> 三态计数见 §1.3（**生效 106 / 拒绝启动 19 / 已读但无效果 31；合计 156**，与 `config/fss.example.json` 的叶子键一一对应）。
...
> 计数由本节的三个清单逐条相加得出：**106 + 19 + 31 = 156**。

| 状态 | 键数 | 说明 |
| --- | --- | --- |
| **生效** | **106** | 读取后真的改变运行行为（含"非法值拒绝启动"的触发条件，写在 §1.2 对应行） |
| **拒绝启动（触发条件）** | **19** | 非默认/不支持的值 → **exit 78（EX_CONFIG）** + 「未实现 + 下一步」 |
