...
>
> 三态计数见 §1.3（**生效 130 / 拒绝启动 15 / 已读但无效果 12；合计 157**，与 `config/fss.example.json` 的叶子键一一对应）。
...
> 计数由本节的三个清单逐条相加得出：**130 + 15 + 12 = 157**。
>
...
| 状态 | 键数 | 说明 |
| --- | --- | --- |
| **生效** | **130** | 读取后真的改变运行行为（含"非法值拒绝启动"的触发条件，写在 §1.2 对应行） |
| **拒绝启动（触发条件）** | **15** | 非默认/不支持的值 → **exit 78（EX_CONFIG）** + 「未实现 + 下一步」 |
| **已读但无效果** | **12** | 组合根未接线（或读了但行为未实现）→ 改它对真实进程没有影响；理由与下一步逐个登记 |
...
> ⚠️ **本清单仍有 12 个键**，它们全部是**能力未实现**（不是"读了没效果"的漏网之鱼）：`server.http.large_file_plane.*`（6，随 sendfile 数据面一起未交付）、`metadata.remote.*`（4，远端元数据仓储未交付）、`metadata.sqlite.max_write_concurrency` 与 `location.sqlite.max_write_concurrency`（单连接 + 互斥 ⇒ 实际写并发恒为 1）。逐条理由与下一步见下方按前缀归类的表。
...
`location.sqlite.max_write_concurrency`、`metadata.remote.base_url`、`metadata.remote.static_token`、`metadata.remote.timeout_ms`、`metadata.remote.token_provider`、`metadata.sqlite.max_write_concurrency`、`server.http.large_file_plane.bind`、`server.http.large_file_plane.max_connections`、`server.http.large_file_plane.port`、`server.http.large_file_plane.sendfile_chunk_bytes`、`server.http.large_file_plane.use_sendfile`、`server.http.large_file_plane.workers`
