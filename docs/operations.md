...
>
> 三态计数见 §1.3（**生效 137 / 拒绝启动 14 / 已读但无效果 6；合计 157**，与 `config/fss.example.json` 的叶子键一一对应）。
...
| 状态 | 键数 | 说明 |
| --- | --- | --- |
| **生效** | **137** | 读取后真的改变运行行为（含"非法值拒绝启动"的触发条件，写在 §1.2 对应行） |
| **拒绝启动（触发条件）** | **14** | 非默认/不支持的值 → **exit 78（EX_CONFIG）** + 「未实现 + 下一步」 |
| **已读但无效果** | **6** | 组合根未接线（或读了但行为未实现）→ 改它对真实进程没有影响；理由与下一步逐个登记 |
...
> ⚠️ **本清单仍有 6 个键**，分两类（都不是"读了没效果"的漏网之鱼）：
> · **能力未实现（4）**：`metadata.remote.*`（远端元数据仓储未交付）；
...
`location.sqlite.max_write_concurrency`、`metadata.remote.base_url`、`metadata.remote.static_token`、`metadata.remote.timeout_ms`、`metadata.remote.token_provider`、`metadata.sqlite.max_write_concurrency`
...
| `metadata` | 5 | `busy_timeout_ms`（切片 3）、`journal_mode`/`synchronous`（**切片 4**）、`group_commit*` 三键（**C10.20**）与 `postgres.{dsn,max_connections,statement_timeout_ms}`（**B1**）已接通（§1.3.1）；`postgres.schema_version_check` 已在 **B2a** 接通（readiness 的 `SELECT 1` + 迁移版本比对，§1.3.1）；`max_write_concurrency`（1）仍未接线 —— 单连接 + 互斥 ⇒ 实际并发恒为 1，改它无可观测效果。仍未接线：`remote.*`（4，远端元数据仓储未交付）。★ `max_write_concurrency` **按决策不投入**（生产用 PG；单连接 + 互斥是有意的正确性设计）—— 不是待办 |
