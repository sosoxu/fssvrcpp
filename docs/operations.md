...
>
> 三态计数见 §1.3（**生效 125 / 拒绝启动 16 / 已读但无效果 15；合计 156**，与 `config/fss.example.json` 的叶子键一一对应）。
...
> 计数由本节的三个清单逐条相加得出：**125 + 16 + 15 = 156**。
>
...
| 状态 | 键数 | 说明 |
| --- | --- | --- |
| **生效** | **125** | 读取后真的改变运行行为（含"非法值拒绝启动"的触发条件，写在 §1.2 对应行） |
| **拒绝启动（触发条件）** | **16** | 非默认/不支持的值 → **exit 78（EX_CONFIG）** + 「未实现 + 下一步」 |
| **已读但无效果** | **15** | 组合根未接线（或读了但行为未实现）→ 改它对真实进程没有影响；理由与下一步逐个登记 |
