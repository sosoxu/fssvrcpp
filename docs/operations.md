...
>
> 三态计数见 §1.3（**生效 141 / 拒绝启动 14 / 已读但无效果 2；合计 157**，与 `config/fss.example.json` 的叶子键一一对应）。
...
| `server.http.large_file_plane.enabled` | ADR-006 独立大文件**下载**数据面（`sendfile`）开关 | `false` | bool；默认 `false` | **生效**（ADR-006）：`false`（默认）→ **不建第二个监听面**，行为与接线前逐字一致；`true` → 组合根创建 `fss::http::LargeFilePlane` 并启动，**启动失败（端口被占等）→ exit 78 + 可读修法**（绝不静默禁用数据面）。跨字段要求：`storage.driver=posix` 且 `self_signed.enabled=true`（数据面复用控制面 `/v1/transfer/{token}` 的校验），否则 exit 78。实际形态在 `/v2/info` 的 `largeFilePlane` 与启动横幅可见 |
| `server.http.large_file_plane.bind` | 大文件数据面监听地址 | `0.0.0.0` | string；默认 `0.0.0.0` | **生效**（ADR-006）：已接通（只认 IPv4 字面量；非法地址 → exit 78 + 修法） |
| `server.http.large_file_plane.port` | 大文件数据面端口 | `8081` | int 1..65535；默认 `8081` | **生效**（ADR-006）：已接通；`bind`/`listen` 失败（端口被占）→ **exit 78 + 可读修法**，绝不静默降级成"数据面关着" |
| `server.http.large_file_plane.use_sendfile` | 是否用 `sendfile(2)`（`false` → 用户态 pump） | `true` | bool；默认 `true` | **生效**（ADR-006）：已接通；`false` 时帧格式**完全相同**，只是多一次用户态拷贝。判据：`/metrics` 的 `fss_large_file_plane_{sendfile,userspace}_{calls,bytes}_total`（P5b 实测 `sendfile` 不动、`userspace` 增长） |
| `server.http.large_file_plane.sendfile_chunk_bytes` | 单次 `sendfile(2)` 的长度上限 | `1073741824` | int ≥65536；默认 `1073741824` | **生效**（ADR-006）：已接通（>2 GiB 的对象必须靠循环，不指望一次调用；1 GiB 传输实测 `sendfile` 字节数恰好等于对象大小） |
| `server.http.large_file_plane.workers` | 大文件数据面工作线程数 | `0` | int 0..65536；默认 `0` | **生效**（ADR-006）：已接通；`0` → `max(64, 2×核数)`（数据面是 IO 密集，**推导与控制面的 `max(16, 4×核数)` 不同**）；启动横幅打印实际值 |
| `server.http.large_file_plane.max_connections` | 大文件数据面在途连接上限 | `0` | int 0..65536；默认 `0` | **生效**（ADR-006）：已接通；`0` → 等于 `workers`；超限 → **503 + `Retry-After: 1`**（与控制面同体；P6 实测第二条连接拿不到对象字节） |
...
| `storage.posix.shared_mount_required` | `multi` 下必须 true（共享挂载探针） | `false` | bool；默认 `false`；`multi` 必须 true | **生效**（B2b，ADR-009 §8.1 item 4）：已接通（`storage.posix.shared_mount_required` / 通用名 `FSS_STORAGE_POSIX_SHARED_MOUNT_REQUIRED`）。`true` → 启动时写 `<storage.posix.root>/.fss_probe.<instance_id>`（内容 = 实例 id + 时间戳）并**读回自证**，随后对 `instance_registry` 里每个 **live peer** 验证其探针文件在本实例 root 下可见：启动期若某 live peer 的探针不可见 → **exit 78**（原因同时给出对端 id、探针路径与两个 root）；运行期每 10s 复查，一旦对端探针不可见 → readiness **not ready**。★ `multi` 仍由 schema 强制该键为 `true`（否则 exit 78）。★ 启动时若尚无 live peer，横幅如实打印"**跨实例可见性尚未验证**"，由运行时心跳复查兜住。**只证明"共享性"（交叉探针可见），不证明 NFS 的 `rename`/close-to-open/`syncfs` 语义（C9.27 仍未验证）** |
| `storage.posix.one_filesystem_per_partition` | 每 partition 独占文件系统（`syncfs` 隔离） | `false` | bool；默认 `false` | **生效**（E1b，ADR-009 §6.3/§8.3）：已接通（`storage.posix.one_filesystem_per_partition`）。`true` = 部署方**断言**"每个 partition 独占文件系统"，组合根在启动期**验证**它（`src/infra/blob/posix/partition_filesystem_check.*`，**只读** `::stat`，跟随符号链接）：**规则 A** 每个 partition 的 staging/persistent 目录必须与 `storage.posix.root` **分盘**（`st_dev` 不同）；**规则 B** 两个不同 partition 不得解析到**同一个**文件系统。违反 → **exit 78**，消息含键名、分区名、完整路径、两组 `st_dev`、`syncfs` 理由与修法。★ **刻意不检查**同一 partition 的 staging 与 persistent 是否同盘（§10.3 允许一个 partition 用两个独立卷）。★ **规则 B 在当前生产形态下是空集**（组合根只由 `partition.file.opendes.*` 构造一个 partition），由合成多 partition 的 L2 用例钉住。★ 该文件系统上是否有**非本服务**负载进程内不可观测，不声称能校验（部署纪律）。`storage.driver=s3` + `true` → **拒绝启动**（s3 下没有可供 `syncfs` 隔离的 partition 文件系统）。目录不存在 → **报错要求先创建/挂载**（本检查不建目录）；`false`（默认）时**不 stat、不拒绝**，行为与接线前逐字一致。横幅 `part fs check :` 打印已校验的设备号（R11：探测结果可见）。证据：`tests/integration/test_partition_filesystem_check.cpp`（U1~U6 + W1~W5）与 `docs/test-evidence/phase10.md` §24 |
...
| JSON 路径 | 含义 | 示例值 | 取值约束 | 接通状态（三态：生效 / 拒绝启动 / 已读但无效果） |
| --- | --- | --- | --- | --- |
| `metadata.repository` | 元数据仓储实现 | `sqlite` | enum `sqlite` / `postgres` / `remote`；默认 `sqlite` | **生效**（B1 + 本切片）：已接通（`metadata.repository`）：`sqlite` → 内置 SQLite（上游没有的本地实现，ADR-004 的"平台外扩展"）；`postgres` → `PostgresMetadataRepository`；`remote` → **`RemoteStorageServiceRepository`（本切片交付）**：记录归**远端 Storage Service** 所有（`PUT/GET {base}/records`、`POST {base}/records/{id}:delete`），因而 OSDU **Search/Indexer** 能看到记录。⚠️ remote 形态的**硬前置**（任一不满足 → **exit 78**，消息点名键与修法）：`deployment.mode=single`（multi 已在 schema 里强制 postgres）+ `metadata.remote.base_url` 非空 + `metadata.remote.token_provider=static` + `leases.enabled=false`（remote 没有 claim，租约空转）+ 本构建有 libcurl。⚠️ 该形态 `capabilities().atomic_claim=false`：**无 C1 原子领取**，并发同一 `FileSource` 的 createMetadata **没有互斥**（可能复制两次并产生两条版本）—— 已登记，见 §1.3.3 与本行下方说明。非法取值由 schema 枚举拒绝 |
...
| `metadata.sqlite.max_write_concurrency` | 有界写并发 | `8` | int 1..256；默认 `8` | **已读但无效果**：写入并发的**上限**；当前实现是「单连接 + 互斥」（实际并发恒为 1 ≤ 上限），改它无从观测（§1.3.3）。**按决策不投入（不是待办）**：这两个 `*.sqlite.max_write_concurrency` 键属于**开发/单实例形态**的调优项 —— 生产用 PostgreSQL（ADR-004 的「内置 SQLite（single）」只是开发与单实例便利），给它做连接池/有界写并发没有生产收益；且「单连接 + 一把互斥」是**有意的正确性设计**（读必须与组提交的批事务互斥，否则会读到未提交数据，见 C10.20 的陷阱条目），不是待优化的实现缺陷。因此本键保持「已读但无效果」，**不再列为下一步**。 |
...
| `metadata.remote.base_url` | 远端 Storage Service 地址 | `""` | string；默认 `""` | **生效**（本切片）：接通 `RemoteMetadataRepositoryOptions.base_url`。★★ **它是「基址」而不是完整端点**：适配器会**追加** `/records`、`/records/{id}`、`/records/{id}:delete`（例如填 `http://host:8080/api/storage/v2`）。⚠️ 这与 `legal.remote.base_url` / `schema.remote.base_url` **正好相反**（那两个是**完整端点 URL**，POST 到它、不追加任何路径，ADR-013 §2）—— 把完整端点填进本键会打到 `.../records/records`（404）。`metadata.repository=remote` 且本键为空 → **exit 78**（fail-closed，消息给示例与修法） |
| `metadata.remote.token_provider` | 远端 token 来源 | `static` | string；默认 `static` | **生效**（本切片）：**只交付 `static`**（静态 `Authorization: Bearer <metadata.remote.static_token>`）。非 `static`（如 `file` / `oauth` / `service-account`）→ **exit 78**（绝不静默当成 static 而以空 token 出站，把认证失败伪装成依赖故障）。⚠️ **OAuth / service-account 换取流程未交付**（登记为未验证项，见 §8） |
| `metadata.remote.static_token` | 静态 token（密文） | `${ENV:FSS_STORAGE_TOKEN}` | string，secret；默认 `""` | **生效**（本切片）：非空 → 每个出站请求带 `Authorization: Bearer <值>`；401 会被适配器映射成 **`kUnauthenticated`（401）**、原因里点名本键（**不是**报成 503 依赖故障）。**绝不打印**（启动横幅只显示 `static_token=***`/`空`）；脱敏键含 `token`，日志里也会打码。空 token 本身**不拒绝启动**（有的内网 Storage 端点允许无鉴权访问），但会被对端 401 |
| `metadata.remote.timeout_ms` | 远端调用超时 | `5000` | int 1..600000；默认 `5000` | **生效**（本切片）：同时约束**连接**与**整体** —— `CURLOPT_TIMEOUT_MS=timeout_ms`，`CURLOPT_CONNECTTIMEOUT_MS=min(1000, timeout_ms)`（与远端 legal/webhook 同一套写法）。超时 → **`kUnavailable`（HTTP 503）**，绝不降级为成功；就绪探针也会因此让 readiness 变 503（liveness 仍 200） |
...
| `location.sqlite.max_write_concurrency` | 有界写并发 | `8` | int 1..256；默认 `8` | **已读但无效果**：写入并发的**上限**；当前实现是「单连接 + 互斥」（实际并发恒为 1 ≤ 上限），改它无从观测（§1.3.3）。**按决策不投入（不是待办）**：这两个 `*.sqlite.max_write_concurrency` 键属于**开发/单实例形态**的调优项 —— 生产用 PostgreSQL（ADR-004 的「内置 SQLite（single）」只是开发与单实例便利），给它做连接池/有界写并发没有生产收益；且「单连接 + 一把互斥」是**有意的正确性设计**（读必须与组提交的批事务互斥，否则会读到未提交数据，见 C10.20 的陷阱条目），不是待优化的实现缺陷。因此本键保持「已读但无效果」，**不再列为下一步**。 |
...
| `leader_election.enabled` | 单例后台任务的领导者选举 | `false` | bool；默认 `false`；`multi` 必须 true | **生效**（B1）：已接通：`true` = 创建 `PgLeaderElection`（**专用锁连接**，只跑短语句）+ 启动时 `pg_try_advisory_lock`；**GC 的周期调度与按需端点都以 `IsLeader()` 门控**（非 leader 不调用 `GcTask::Run` ⇒ `fss_gc_runs_total` 不增加）。创建/取锁失败 → **exit 78**。`false` = 不做门控（单实例语义逐字不变）。横幅新增 `leader` 行给出 backend/lock_key/启动时角色。★ **运行期**（ADR-009 §4.4.1）：会话级锁与会话同生共死 —— 锁连接被切断（**PG 重启 / 主备切换 / 网络抖动**）时本实例立即降级，并在下一轮**自动重连并重新参与选举**（重连尝试带 1s 冷却）；期间日志用独立事件 `gc_skipped_leader_election_unavailable` 与"别人是 leader"区分，`--once` 在此形态下返回 **78** 而不是伪装成正常跳过 |
...
> ★ **本清单里 `storage` 前缀已清空**：最后一个 storage 键 `storage.posix.one_filesystem_per_partition` 已在 **E1b** 移入「生效」（§1.3.1：启动期用 `::stat` 验证"每个 partition 独占文件系统"，不成立 → exit 78）。
> ⚠️ **本清单仍有 2 个键**（都不是"读了没效果"的漏网之鱼）：
> · **按决策不投入（2）**：`metadata.sqlite.max_write_concurrency` 与 `location.sqlite.max_write_concurrency` —— 生产用 PG（SQLite 只是开发/单实例形态），且「单连接 + 互斥」是有意的正确性设计（C10.20），**这两个键不列为待办**（理由见 §1.2 对应行）。
> 逐条理由见下方按前缀归类的表。
> B2b 让 `storage.posix.shared_mount_required` **真正生效**（§1.3.1），但那只证明"交叉探针可见 = 共享性"，**不证明** NFS 的 `rename`/close-to-open/`syncfs` 语义（C9.27 仍未验证）。
...
> **B2b（本切片）的净变化**：`storage.posix.shared_mount_required` 从本清单移入「生效」（+1，§1.3.1）；三态从 **128/15/14** 变为 **129/15/13**（键数仍 157）。实现：① 真的用 PG 时 upsert `instance_registry`（`instance_id`/`service_version`（= `FSS_BUILD_VERSION`）/`config_hash`（**脱敏**有效配置去掉每实例键后的 SHA-256）/`started_at`/`heartbeat_at`），后台线程每 **10s** 刷新心跳；② live peer = 心跳在 **3×10s = 30s** 内，其 `config_hash` 或 `service_version`（major.minor）与本实例不一致 → readiness **not ready**（滚动升级护栏）；stale 行被**忽略**（启动期按 300s 阈值清理过老行）；③ `shared_mount_required=true` → 写 `<root>/.fss_probe.<instance_id>` 并对每个 live peer 交叉验证探针可见性：启动期不可见 → **exit 78**（原因给出对端 id/探针路径/两个 root），运行期不可见 → not ready。readiness **复用 B2a 的** `ports.shared_state_probe`（REST/gRPC 同源）。测试接缝 `FSS_SERVICE_VERSION_OVERRIDE`（**环境变量，不是配置键**）。⚠️ 只证明"共享性"（交叉探针可见），**不证明 NFS 语义（C9.27 仍未验证）**。
> **E1b（本切片）的净变化**：`storage.posix.one_filesystem_per_partition` 从本清单移入「生效」（+1，§1.3.1）；三态从 **129/15/13** 变为 **130/15/12**（键数仍 157）。实现：新增 L2 `src/infra/blob/posix/partition_filesystem_check.*`（**只读** `::stat`，跟随符号链接），组合根在 `StaticPartitionRegistry` 之后按 `ObjectKeyPolicy::ContainerFor` 解析容器目录并校验：**规则 A**（每个 partition 的 staging/persistent 必须与 `storage.posix.root` **分盘**）+ **规则 B**（两个 partition 不得解析到同一文件系统）；违反 → **exit 78**，消息含键名/分区名/完整路径/两组 `st_dev`/`syncfs` 理由/修法。`storage.driver=s3` + 该键 `true` → **拒绝启动**（s3 下没有可供隔离的 partition 文件系统）。`false`（默认）时**不 stat、不拒绝**，判定行为逐字不变。★ **刻意不检查**同一 partition 的 staging 与 persistent 是否同盘（§10.3 允许两卷）；★ **规则 B 在当前生产形态下是空集**（组合根只构造一个 partition），由合成多 partition 输入的 L2 用例钉住；★ 该文件系统上是否有非本服务负载**不可观测**，不声称能校验。真实进程证据与 R1 注入见 `docs/test-evidence/phase10.md` §24。
> **ADR-006（本切片）的净变化**：`server.http.large_file_plane.{enabled,bind,port,use_sendfile,sendfile_chunk_bytes,workers,max_connections}` —— **7 个键**（1 个来自「拒绝启动」+ 6 个来自本清单）全部移入「生效」（§1.3.1）；三态从 **130/15/12** 变为 **137/14/6**（键数仍 157）。实现：L1 `bytes::ByteSource::NativeFd()`（可选零拷贝能力，默认 `-1` = 必须回退用户态）+ L2 `src/infra/transfer/posix_file_byte_source.*`（`pread` + 自有偏移 + RAII `::close`，`Seek` 边界与 `BlobByteSource` 逐条一致）+ 端口默认方法 `IBlobStore::OpenNativeRead`（**只有 POSIX 覆盖**，内存/S3 得 `kUnimplemented`；`MeteredBlobStore` 转发并给用户态拷贝计量）+ L1 `src/common/http/large_file_plane.*`（**进程内第二个监听 socket**；复用 `Router::BuildTransferGetHandler()` 的**同一个**已包装 handler ⇒ 鉴权/租户/错误体/HTTP 指标同源；访问日志调**同一份** `fss::http::LogAccess`，解析错误体调**同一份** `ErrorBody`）+ 组合根装配（`open_get` 优先 `OpenNativeRead`）。⚠️ **范围与差异（显式登记）**：只服务 `GET/HEAD`（`PUT` 留在控制面 → 数据面 405；未知路径 404）；**多段 Range 降级为 200 全量**（控制面对可寻址来源是 multipart 206 —— 这是**唯一**被文档化的行为差异）；超长 URI 数据面回 **414**，而控制面因 httplib 的 header reader 先失败而**不返回任何 HTTP 响应**（实测差异，已登记，见 `docs/test-evidence/phase10.md` §25）。⚠️ **计数点移动**：`sendfile` 绕过 `get()` ⇒ 该路径的字节/操作数由数据面补记（`MeteredBlobStore::RecordNativeRead`，与 `get()` 同族同标签）；用户态拷贝仍由装饰器记。⚠️ **未验证**：ADR-006 §6.5 的真实存储/网卡受控复核（本环境是 WSL2 + 虚拟盘 + loopback，绝对值**无结论**；脚本 `scripts/bench_sendfile_ab.sh` 已移交生产环境）、TLS/mTLS（ADR-006 §7.3 的重开条件）、真实跨主机/多实例数据面、>2 GiB 对象（本切片实测最大 **1 GiB**）；`splice`/上传加速未做。
> **`metadata.remote.*`（本切片）的净变化**：`metadata.remote.{base_url,token_provider,static_token,timeout_ms}` —— **4 个键**全部从本清单移入「生效」（+4，§1.3.1）；三态从 **137/14/6** 变为 **141/14/2**（键数仍 157），本清单只剩 2 个**按决策不投入**的 `*.sqlite.max_write_concurrency`。实现：L2 `src/infra/metadata/remote/remote_metadata_repository.{h,cpp}`（libcurl；fail-closed 错误映射；`Ready()`/`NotReadyReason()`）+ 用例的**能力分支**（`capabilities().atomic_claim`）+ 组合根四条 fail-closed 前置与就绪探针 + 指标 `fss_metadata_remote_requests_total{op,outcome}`（只在 remote 形态注册）。
> ⚠️ **两条必须让运维知道的设计偏离（不是缺陷）**：
> · **确定性记录 id**：`remote` 形态的记录 id = `<partition>:dataset--File.Generic:<sha256(partition\0fileSource) 前 128 bit 的 8-4-4-4-12 形状>`（R5：幂等键必须建在幂等键上）。上游让 Storage Service 分配随机 id，本项目**有意偏离**；因此**不要手工编辑 Storage Service 里的记录 id**（见 `docs/runbook.md` §2.1 的禁令）。
> · **无原子领取（`atomic_claim=false`）**：远端没有 ADR-009 §4.2 的"条件插入"原语与 `state` 列 ⇒ **并发同一 `FileSource` 的创建没有互斥**，可能复制两次并产生**两条版本**（对象物理位置相同 ⇒ 不丢数据）。因此 remote **仅单实例**（组合根强制 `deployment.mode=single` + `leases.enabled=false`，否则 exit 78）；生产多实例必须用 `metadata.repository=postgres`。

`location.sqlite.max_write_concurrency`、`metadata.sqlite.max_write_concurrency`

**理由与下一步（按前缀归类）**

| 前缀 | 键数 | 为什么不生效 / 下一步 |
| --- | --- | --- |
| server.http | 0 | **ADR-006 已清空**：`large_file_plane.{enabled,bind,port,use_sendfile,sendfile_chunk_bytes,workers,max_connections}` 全部接通（§1.3.1：第二个监听面的开关/绑定/端口/零拷贝开关/单次长度/线程/连接上限）；`transfer_max_body_bytes` 已在**切片 4** 接通（§1.3.1） |
| `storage` | 0 | **E1b 已清空**：最后一个键 `one_filesystem_per_partition` 已接通（§1.3.1：启动期按 partition 校验文件系统隔离，违规 → exit 78；`storage.driver=s3` + `true` → 拒绝启动）。B2b 已把 `shared_mount_required` 接通（§1.3.1：写探针 + 与 `instance_registry` 的 live peer 交叉验证可见性；启动期不可见 → exit 78，运行期不可见 → not ready）。C10.16 续已把 `atomic_write`/`dir_mode`/`file_mode`/`fadvise_random`/`fadvise_dontneed_after_large_read` 接到驱动层（§1.3.1）；`group_commit_max_batch` 也已接通（ADR-008 的 P4，§1.3.1），`sync_dir_after_batch=false` 保持拒绝启动（R2 不变量，§1.3.2） |
...
| `metadata` | 5 | `busy_timeout_ms`（切片 3）、`journal_mode`/`synchronous`（**切片 4**）、`group_commit*` 三键（**C10.20**）与 `postgres.{dsn,max_connections,statement_timeout_ms}`（**B1**）已接通（§1.3.1）；`postgres.schema_version_check` 已在 **B2a** 接通（readiness 的 `SELECT 1` + 迁移版本比对，§1.3.1）；`max_write_concurrency`（1）仍未接线 —— 单连接 + 互斥 ⇒ 实际并发恒为 1，改它无可观测效果。`remote.*`（4）已在**本切片**接通（§1.3.1：远端 Storage Service 仓储；⚠️ `atomic_claim=false`、仅单实例、无互斥）。★ `max_write_concurrency` **按决策不投入**（生产用 PG；单连接 + 互斥是有意的正确性设计）—— 不是待办 |
| `location` | 1 | `busy_timeout_ms`/`journal_mode`（切片 3）、`synchronous`（**切片 4**）、`group_commit*` 三键（**C10.20**）与 `postgres.{dsn,max_connections}`（**B1**）已接通（§1.3.1）；只剩 `max_write_concurrency`（1）：单连接串行实现下它不改变行为，且**按决策不投入**（生产用 PG）—— 不是待办 |
...
| `GET /api/file/v2/readiness_check` | 依赖就绪 | 探针 = 组合根注入的 `ports.shared_state_probe`（**真的用 PG 时**：PG `SELECT 1` + `schema_migrations.max(version)` 比对（B2a）+ `instance_registry` 心跳/共享挂载/版本/config_hash 一致性（B2b）），两条协议同源；未用 PG 时退回元数据仓储 `List("__readiness__")`。成功 `200` 纯文本 `File service is ready`；失败 `503` 纯文本 `File service is not ready: <可读原因>` |
...
> `readiness_check`（真的用 PG 时）**不**探存储/磁盘水位：它探 PG 存活 + 迁移版本 + 实例一致性（共享挂载探针/版本/config_hash）；磁盘满时它仍可能返回 200（见 §6.1）。未用 PG 时退回"元数据仓储可达"最小探针。
...
**远端元数据仓储（`metadata.repository=remote`；`RemoteMetadataRepository` 直接写注册表）**

| 指标名 | 类型 | 标签 | 含义 |
| --- | --- | --- | --- |
| `fss_metadata_remote_requests_total` | counter | `op` ∈ `put` / `get` / `delete` / `probe`、`outcome` ∈ `ok` / `error` | 发往远端 **Storage Service** 的出站请求数。`op` 直接指路：`put`=写入（createMetadata）、`get`=读/幂等预检、`delete`=删除、`probe`=**就绪探针**（只有它失败才会让 readiness 变 503）。★ **只在 `metadata.repository=remote` 时存在**（默认 `sqlite` 形态的 `/metrics` **没有**这一族，见 `tests/integration/test_remote_metadata_repository.cpp` 的 M12 断言）；远端仓储的**每一次失败**还会在日志里留一条 `warn`（含 op + HTTP 状态 + error kind） |
...
* `syncfs` 是**文件系统级**操作，会 flush 整个文件系统；多业务共盘会互相牵连 →
  ...
  `storage.posix.shared_mount_required`）。**B2b 起 `shared_mount_required` 真的生效**：
  启动时写 `<root>/.fss_probe.<instance_id>` 并与 `instance_registry` 的 live peer 交叉
  验证探针可见性（启动期不可见 → exit 78；运行期不可见 → not ready）—— 它证明的是
  **共享性**，**不是** NFS 的 `rename`/close-to-open/`syncfs` 语义（C9.27 仍未验证，§8）。
  **E1b 起 `one_filesystem_per_partition` 也真的生效**：`storage.driver=posix` 且该键为
  `true` 时，组合根在启动期按 partition 校验 `st_dev`（规则 A：与 `root` 分盘；规则 B：
  partition 之间不共盘；§1.3.1 与该键的 §1.2 行），断言不成立 → **exit 78**；
  `storage.driver=s3` + `true` → **拒绝启动**（没有可供隔离的 partition 文件系统）。
...
**症状**：liveness `200`，readiness `503` + 纯文本 `File service is not ready: <原因>`。
**原因**：真的用 PG 时，readiness = PG 存活（`SELECT 1`）+ 迁移版本比对（B2a）+ **实例一致性**（B2b：每个 live peer 的共享挂载探针可见 / `service_version` 兼容 / `config_hash` 一致）；未用 PG 时退回元数据仓储 `List("__readiness__")` 最小探针。**不是**磁盘水位探针。

**诊断顺序**：① 读 503 文本里的**可读原因**（它已指出是对端实例 id、探针路径、版本还是 config_hash）；② 用 §6.2 检查元数据库；③ 看日志有没有仓储错误；④ 确认 `FSS_METADATA_SQLITE_PATH` 指向的文件系统还有空间；
⑤ 确认没有另一个进程以写方式独占该库。
...
| `metadata.repository`/`location.repository` 的 `postgres` | **已实现（B1）** | 组合根创建 `PostgresMetadataRepository` / `PostgresLocationRepository`，DSN 不可达 → **exit 78**（+ libpq 原文），绝不静默降级为 SQLite。`metadata.repository=remote` **已实现（本切片）**：远端 Storage Service 仓储，四条前置（single / base_url / token_provider=static / 无租约）任一不满足 → exit 78（§1.2.7） |
...
| `metadata.sqlite.max_write_concurrency` / `location.sqlite.max_write_concurrency` | **未接通（如实登记）** | 两个仓储都是「单连接 + 互斥」⇒ 实际写并发恒为 1 ≤ 上限，改它不改变行为。★ **按决策不投入**：生产用 PostgreSQL，SQLite 只是开发/单实例形态；且「单连接 + 互斥」是有意的正确性设计（`group_commit*` 三键已由 **C10.20** 接通，见 §1.3.1） |
...
| 远端 Storage Service 仓储（`metadata.repository=remote`） | **已实现（本切片）；真实服务未联调** | ADR-004 的可选实现。L2 `src/infra/metadata/remote/remote_metadata_repository.{h,cpp}`：`PUT {base}/records` / `GET {base}/records/{id}` / `POST {base}/records/{id}:delete`（**必须 204**；ADR-004）；`base_url` 是**基址**（追加路径，与 `legal/schema.remote.base_url` 相反）；**确定性记录 id**（R5）；`capabilities().atomic_claim=false`（并发同一 `FileSource` **无互斥**，可能两条版本）；组合根四条前置（single / base_url / `token_provider=static` / 无租约）+ 就绪探针（依赖挂 → readiness 503、liveness 200）。⚠️ **未与真实 Storage Service 联调**：线协议按 `docs/01-osdu-research.md` 的端点写、由 `mock_validators.py --mode storage` 钉住；响应形状（整条记录 JSON / OSDU `{recordCount,recordIds,versions}` 信封）都被接受但只被 mock 覆盖；**`token_provider` 只交付 `static`**（OAuth / service-account **未实现**）；`DataLakeStorageService`/core-common 中介**未实现**（适配器直接调 Storage 端点）；remote **仅单实例**（不支持 multi）。证据 `docs/test-evidence/phase10.md` §26 |
...
| **`syncfs` 全局 flush 对他人的影响（C9.24）** | **未验证（干扰量级）** | `syncfs` 是文件系统级操作：多租户共盘会牵连他人写入。本环境没有多租户共盘场景，**干扰量级仍无法测量**。缓解方向（按 partition 分盘）**已在 E1b 接通**：`storage.posix.one_filesystem_per_partition=true` 时组合根启动期校验规则 A/B，断言不成立 → exit 78（§1.3.1）。★ 区分清楚：该键只保证**"分盘这件事成立"**，**不**测量 C9.24 的干扰量级 |
...
| sendfile 大文件数据面 | **已交付（下载面，ADR-006）** | 进程内第二个监听面（方案③）：`GET/HEAD /<base>/v1/transfer/{token}`，复用控制面**同一个**已包装 handler；`enabled=false` 默认关闭。⚠️ **§6 第 5 条（真实存储/网卡受控复核）未完成**；>2 GiB 未实测（最大 1 GiB）；TLS 未评估 |
...
- ★ **`storage.posix.one_filesystem_per_partition` 已接通（E1b）**（§1.3.1）：`true` 时组合根在启动期
  校验"每个 partition 独占文件系统"（规则 A：与 `root` 分盘；规则 B：两个 partition 不共盘），
  违规 → **exit 78**；它把"按 partition 分盘"从**纯部署纪律**变成**可被拒绝启动的断言**
  （但仍**不**测量 C9.24 的干扰量级，那是另一个未验证项）。
...
| `syncfs` 跨实例干扰量级（C9.24） | **未验证** | 只能靠部署纪律分盘（§10.3）；`one_filesystem_per_partition` 已在 **E1b** 接通（启动期校验规则 A/B，违规 exit 78），但**该键不测量干扰量级** |
| 大文件下载数据面的**多实例 / 跨主机**形态（ADR-006） | **未验证** | 数据面在**单进程 + loopback**上验证（`tests/integration/test_large_file_plane.cpp`）；跨主机 LB 粘性、多实例下数据面的租约/GC 约束未单独复测。真实存储/网卡上的受控基线（ADR-006 §6 第 5 条）**未完成**（脚本 `scripts/bench_sendfile_ab.sh` 移交生产）；TLS/mTLS 未评估（ADR-006 §7.3） |
