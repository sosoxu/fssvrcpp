// 独立进程负载生成器：N 条 keep-alive 连接，持续请求 T 秒
#include <arpa/inet.h>
...
struct C{int fd=-1;std::string buf;
  ...
  bool one(){const char* r="GET /s HTTP/1.1\r\nHost: h\r\n\r\n";
    ...
    for(;;){size_t he=buf.find("\r\n\r\n");if(he==std::string::npos){if(!pump())return false;continue;}
      ...
      while(buf.size()<need)if(!pump())return false;
      buf.erase(0,need);return true;}}
};
int main(int argc,char**argv){
  ...
  printf("T=%-5d -> %9.0f req/s  ok=%lld err=%lld\n",T,double(cnt.load())/secs,cnt.load(),err.load());
  return 0;
}
