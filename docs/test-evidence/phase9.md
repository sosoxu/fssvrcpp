...
## 16. C9.27 探针交付 + **本环境无法执行**（如实登记，非"已验证"）

**结论先说**：C9.27（目标存储的 NFS 语义）**仍未验证**。本轮交付的是**可复跑的判据**
（`scripts/check_nfs_semantics.sh`，已登记进 `AGENTS.md` §5 的机械检查表），并把执行**移交生产环境**。

### 16.1 为什么开发环境做不了（实测证据，不是推测）
| 检查 | 结果 |
| --- | --- |
| 已挂载的网络/共享类文件系统 | **无 NFS/CIFS/Ceph/sshfs**；只有 `9p`（WSL 的 `C:`/`D:`）、`fuse.snapfuse`、`tmpfs`、`overlay`、`ext4` |
| 运行环境 | WSL2（`6.18.33.2-microsoft-standard-WSL2`），`id -u` = 1000（**非 root**） |
| `/etc/fstab` | 无网络挂载项 |
| NFS 客户端工具 | `mount.nfs` / `mount.nfs4` / `showmount` **全缺**（`smbclient`/`sshfs`/`rclone` 亦缺） |
| 已知局域网主机（`172.17.64.1`，即 PG 主机与默认网关） | `2049`/`111`/`445`/`139`/`3260`/`6789` **全不通** |
| 网段扫描 `172.17.64.0/24:2049` | **无主机开放**（该网段没有 NFS 服务端） |

`mount` 需要 `CAP_SYS_ADMIN`，且缺少客户端工具 ⇒ **不存在**"绕过 root 用用户态 NFS"的路径。
（本环境是**开发**环境：生产只在 Linux、数据库也在 Linux；NFS 语义由运维在**生产存储**上执行验证。）

### 16.2 探针自身已自证"真的跑过、且判据能失败"（R1）
* 本地 ext4 实跑：N1/N2/N3 **全通过**（N1 有效采样 365 次、N2 延迟 2–3 ms）、退出码 **0**。
* **注入消融**：把原子 `mv` 换成非原子 `cp` → **N1 失败**（非期望内容 12 次）、退出码 **1**；
  `md5` 逐字还原后再次全绿 ⇒ 判据有区分力，不是恒真。
* 诚实边界写在脚本里：**断电耐久性**为信息性说明（不计入"无结论"计数），否则一条永远测不了的项
  会让退出码永远为 2、失去意义（R4）。

### 16.3 移交清单（生产环境执行时才关闭本条）
1. 在**挂载了共享存储的机器**上跑 `./scripts/check_nfs_semantics.sh <mount>`（不需 root，约 30 s）；
2. **两台主机各一次**（跨主机 close-to-open 才被覆盖）；
3. 回贴**完整原始输出**（含文件系统类型/挂载选项）→ 由开发方登记进本文件，并把 C9.27 从"未验证"改为有据结论；
4. 仅在**字节面走共享 POSIX 挂载**的形态下需要做；若字节面是 S3/对象存储，则本项**不适用**（ADR-009 §4.1）。
